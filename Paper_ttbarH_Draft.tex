...
plausible within the short term LHC is considered. {\bf (KK: reword last sentence a bit.)}
...
It is of particular importance to test the coupling of the putative ({\bf (KK: at some point
...
the Higgs mass -- and it may play an important role in the EWSB {\bf KK: define EWSB?}
mechanism.  {\bf (KK: I reworded a lot of this paragraph -- is it still
  correct, or did I accidentally alter the meaning?)}\par
...
\cite{constraints1,constraints2, constraints3,constraints4}.
     {\bf (KK: OK to drop ``gluon fusion'' here, since we already said it was
...
constraints derived from measurements of Higgs boson decay rates
{\bf (KK: correct to add ``boson decay'' here?)} are indirect
constraints. Electric dipole moments (EDMs) {\bf (KK: delete ``EDM'' since we
  never use it again?)} can also
...
{\bf (KK: did I phrase this correctly, calling it ``associated production'' in this way?)}
...
$t\tbar$ pair has been studied by several authors {\bf (KK: true?)}, and
...
to name a few \citep{Guadagnoli}. Also, an approach based on weighted
...
{\bf (KK: is this last one CP even, too?)}  Now, $\mathrm{CP}$-even observables are not
...
As is well known {\bf (ref)}, in the case of semileptonic top decay,
the charged lepton is the most powerful in this regard.
It is also known that the top quark and antiquark spins
are highly correlated in $t\tbar$ production\cite{Mahlon1,*Mahlon2,*Mahlon3,*atwood},
a feature that
is manifested in the double angular distributions of the decay
products of the $t$ and $\tbar$ systems {\bf (ref)}.
...
the top couples to the Higgs boson. {\bf (KK: Did I simplify this too much?
...
involve the Higgs boson. {\bf (KK: did I preserve the physics meaning
...
expression for the differential cross section for {\bf (name of process)},
...
sensitive to the couplings. {\bf (KK: can we simplify the previous
  sentence?)}  Further, the TPs correlations in these
...
the last one depicts the $gg$ $t$-channel diagrams. By exchanging the gluon
lines in the third row, three more $gg$-initiated diagrams can be
obtained. We describe the $tH$ coupling with the effective Lagrangian
%
\beq
\label{eq1}
...
and $\kpt\neq 0$ parameterize a $\mathrm{CP}$-odd Higgs. \par
%%%%%%%%%%%%%%%%%%%% Figura 1%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
semileptonically.  By using the narrow-width
...
cross section for the production of a top quark with spin vector $n_t$
plus an anti-top quark with spin vector $n_{\bar{t}}$ and a Higgs
boson, and $d\Gamma(t\to b{\ell}^+\nu_{\ell})$ and
$d\Gamma(\tbar \to \bbar {\ell}^-\nubar)$ are the
differential decay widths for an unpolarized top and
anti-top, respectively. Also, the vectors
...
We will outline now how to derive
