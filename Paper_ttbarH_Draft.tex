...
\email{\hlkk{dkcrane@mtu.edu}}
\tayloru

\author{Ethan Gegner}
\email{ethan_gegner@taylor.edu}
\tayloru
...
are based on triple products (TP) structures that we extract from the
expression of the differential cross section in terms of the spin
vectors of the top and antitop quarks. In order to explore other
possibilities, we progressively modify these TPs, firstly, by
combining them and then by replacing the spin vectors by the lepton
momenta or the $t$ and $\tbar$ momenta by their visible parts. We
obtain that the most promising observable can disentangle the
hypotheses $\kp=1,\kpt=\pm 1$ by more than $\sim 20\sigma$ for $10^5$
events that is within the expected order of magnitude of events at the
HL-LHC. In the case of observables that do not need the reconstruction
of the $t$ and $\tbar$ momenta, the power of discrimination is up to
$\sim 16\sigma$ for the same ammount of events. We also show that the
...
its characterization, namely the determination of its physical
properties with the highest possible precision. Among the features of
the new boson that are under investigation, the study of its couplings
to the fermions is of great relevance, allowing us to study the nature
of $H$ under the discrete symmetry $\mathrm{CP}$ and to clarify the
extent to which this new particle is consistent to the Higgs boson
predicted by the Standard Model (SM) of particle physics. In
particular, the test of the Higgs coupling to the heaviest fermion,
the top quark, is a priority since on the one hand, it rules the main
production mechanism through gluon fusion and contributes to the
important decay mode to two photons, and on the other hand, it is
...
the Higgs mass and it may play an important role in the EWSB
mechanism.\par

Given the fact that the main Higgs boson production
process, the gluon fusion, is dominated by a top quark loop and that
...
\cite{constraints1,constraints2, constraints3,constraints4}. However,
these constraints assume that there are no other sources contributing
to the corresponding effective couplings and, in the case of the
diphoton decay channel which also involves a $W$ boson loop, that the
...
constraints. In this direction, electric dipole moments (EDM) can also
...
directly the $tH$ coupling, processes with smaller cross sections need
...
factor, we will concentrate in the following combination arising from
