...
form given in Eq.~(\ref{eq27}). The ascociated shifts ($\delta$)
...
as can be seen from Table \ref{table1}. {\bf (N: We have followed your proposal but
also added that the asymmetry for this TP, which is the one proposed by Ellis, is slightly less sensitive
 than in the case of $\epsilon_2$. This is in order to emphasize that we tend to prefer the latter (even when
 the improvement in the sensitivity is small in terms of the statistical uncertainty).) }
\par
...
in Ref.~\cite{Guadagnoli}. {\bf (KK: Is this lab frame observable
  ...
  my confusion on this!) (N: They compute the asymmetry of a quantity that is basically the product
  of an angle and the sign of a TP)}  We have found that this observable
appears to
...
neutrino weighting technique along with associated kinematic constraints {\bf (N:add references)}.
