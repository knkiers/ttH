...
the angle $\Delta\phi_{\ell^-\ell^+}$ [see the discussion following
  Eq.~(\ref{eq28}) {\bf (KK: should this be Eq~(\ref{eq25})?)}],
...
[via Eq.~(\ref{eq26})], {\bf (KK: correct, I think\ldots?)}
...
distribution and somewhat smaller than those obtained for the
$\Delta\phi_2(n_t,n_{\tbar})$ distribution.
{\bf (KK: A few comments\ldots (i) From Table I, it doesn't seem like
...
the asymmetry. 
{\bf (KK: I don't know that we should separate $\epsilon_2$ from
...
Compared to the TPs $\epsilon_1$-$\epsilon_3$ and
...
smaller for the
$\epsilon_6$ and $\epsilon_7$ distributions.  {\bf (KK: rewording OK/correct?)}
Taking into account these facts we do not give further
details of the angular distributions for the TPs discussed in this
section.
{\bf (KK: I would suggest deleting this last sentence.)}
\par
...
{\bf (KK: note that I changed the reference from table3 to sec3.)}
...
\caption{Asymmetry for the TP $\epsilon_{9}$ for the SM case and the
  two $\mathrm{CP}$-mixed cases defined by $\kp=1,\kpt=\pm 1$. The
  values are obtained with $10^5$ simulated events.
  {\bf (KK: I repositioned the table in the file, please double check to make
...
to test the asymmetry corresponding to the lab frame observable given
in Ref.~\cite{Guadagnoli}. {\bf (KK: Is this lab frame observable
  ...
  my confusion on this!)}  We have found that this observable
{\bf (KK: ``asymmetry''?)}
appears to
be slightly less sensitive than the combination $\epsilon_{10}$,
{\bf (KK: ``$\mathcal{A}(\epsilon_{10})$''?)}
...
more attainable in the near future.  {\bf (KK: OK?)}
...
could be either electrons or muons, {\bf (KK: factor of 4\ldots?)}
the SM cross section for $\ppprocess$ at
...
events increases to $\sim 5.49 \times 10^4$.
{\bf (KK: should we use 5.5 at this point, since
...
associated with their respective parent quark~\cite{atlasconf}.
{\bf (KK: maybe we can talk about this for a minute; I don't think I
...
but also because the (visible) quarks and charged leptons in the final
...
vertices involved in the process. {\bf (KK: Can we either simplify the
...
Within the context of $t\tbar$ production {\bf (KK: correct?)} this
...
$t\tbar H (H\rightarrow b\bbar)$~\cite{dosSantos}.
{\bf (KK: do we need an initial state
  on this process?)} In all these cases the corresponding
efficiency in the reconstruction of the momenta is up to $\sim 80\%$.
{\bf (KK: Does this mean that none of them exceeds 80\% or that
