...
with an effective separation of $\sim 19\sigma$. In
...
13\sigma$ for the same number of events. We also show that the
...
triple product (TP) correlations that we extract from the
...
observables based on TPs that do not contain the $t$ and $\tbar$
spin vectors, and
in certain cases incorporate the Higgs momentum, are
discussed in Sec.~\ref{sec4}; observables that do not
...
\psi_{\tbar} = g^2\, \mathbb{P}_t(\tbar)\,\mathbb{P}_W(\tbar-\bbar)\,
   \langle\nubar+|\bbar-\rangle (\tbslash-m_t)|{\ell}^-+\rangle,\\[1mm]
\eeq
%\bea
%\label{eq7}
%\bar{\psi}_t &=& -g^2\, \mathbb{P}_t(t)\,\mathbb{P}_W(t-b)\,
%   \langle b-|\nu_{\ell}+\rangle \langle {\ell}^+\!+|(\tslash+m_t)\\[0.4cm]
%\label{eq8}
%\psi_{\tbar} &=& g^2\, \mathbb{P}_t(\tbar)\,\mathbb{P}_W(\tbar-\bbar)\,
...
always yield smaller values for the $\chi^2/\mathrm{d.o.f}$.\par
...
become distorted in such a way that there is a non-zero
asymmetry [see Eq.~(\ref{eq26})].  Moreover, the associated asymmetry values are
...
are clearly disentangled, since the observables are
...
Sec.~\ref{sec3.1} is slightly less sensitive than $\langle \epsilon_1\rangle$,
 $\langle \epsilon_2\rangle$ and $\langle \epsilon_3\rangle$, 
 with values $-4.32$, $1.11$ and $7.23$ for the cases
...
the sensitivity to the anomalous $tH$ coupling is smaller for the mean values
...
\label{sec4}
So far we have considered TPs involving the momenta $t,\tbar$
...
We have also studied the associated angular distributions. Specifically, 
we have analyzed the $\Delta\phi(\ell^+,\ell^-)$ distribution in the $t\tbar H$ rest frame,
taking $H$ to define the $z$-axis. The distributions obtained for different values of
$\kp$ and $\kpt$ are not well described by Eq.~(\ref{eq27}) (the situation is similar
to that encountered for the angular distribution
associated with $\epsilon_4$ -- see the discussion
at the end of Sec.~\ref{sec3.2}.).  For different values of the parameters,
the distributions become slightly distorted giving rise to a non-zero asymmetry 
[see Eq. (29)].
\begin{table}[H]
...
Comparing
Tables~\ref{table9} and \ref{table10}, we see that the effective
...
giving rise to an effective separation between the $\mathrm{CP}$-mixed scenarios
...
scenarios under consideration is $6.5\sigma$. \par
...
\end{center} 
\end{table}
The results shown in Tables \ref{table11} and \ref{table12} reveal that
...
LHC. We note that in order to be fully conclusive about  
%
\begin{table}[H]
...
%
\noindent
the required luminosity, it is important to include the effects of hadronization,
...
decreased by $3.1\sigma$ compared to the asymmetry associated
with $\epsilon_4$ for
