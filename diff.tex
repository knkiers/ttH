...
%DIF ADD Paper_ttbarH_Draft.tex                              Thu Mar  3 20:28:22 2016
\pdfoutput=1
\usepackage{graphicx}
%\usepackage{dcolumn}
\usepackage{amsfonts}
...
%DIF > 
\DIFaddend \section{Experimental Feasibility}
\label{sec6}
%
\DIFdelbegin \DIFdel{By considering the mild selection cuts introduced in }\DIFdelend \DIFaddbegin 

\DIFadd{In our numerical analyses so far we have used relatively large
samples of events ($10^5$ events per sample)
in order to clearly distinguish
which observables would be most promising.  
The number of events expected at the HL-LHC, however, is
...
more attainable in the near future.  }{\bf \DIFadd{(KK: OK?)}}

\DIFadd{Let us first make some estimates regarding the number of signal events
expected at the HL-LHC.
In }\DIFaddend Sec.~\ref{sec3} \DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{we introduced several mild selection cuts.
Implementing these cuts, and assuming that the final state leptons
could be either electrons or muons, }{\bf \DIFadd{(KK: factor of 4\ldots?)}}
\DIFaddend the SM cross section for $\ppprocess$ \DIFdelbegin \DIFdel{, $\ell=e,\mu$ }\DIFdelend at
$14\,\mathrm{TeV}$ is $\sim 15.3\,\mathrm{fb}$\DIFdelbegin \DIFdel{and hence }\DIFdelend \DIFaddbegin \DIFadd{; thus, }\DIFaddend the number of
events expected within the context of the HL-LHC is $\sim
15.3\,\mathrm{fb} \times 3000\,\mathrm{fb}^{-1} = 4.59\times
10^4$. This number is expected to be \DIFdelbegin \DIFdel{even larger
in the case of
$\kp=1,\kpt\neq 0$ }\DIFdelend \DIFaddbegin \DIFadd{larger
if $\kpt\neq 0$ (assuming $\kp=1$),
}\DIFaddend since the corresponding cross section is
\DIFdelbegin \DIFdel{then
higher }\DIFdelend \DIFaddbegin \DIFadd{larger }\DIFaddend than the SM \DIFdelbegin \DIFdel{one.
By taking }\DIFdelend \DIFaddbegin \DIFadd{cross section in this case.
Taking }\DIFaddend into account NLO corrections \DIFaddbegin \DIFadd{(}\DIFaddend to the
production process\DIFdelbegin \DIFdel{and considering }\DIFdelend \DIFaddbegin \DIFadd{) via }\DIFaddend a $K$ \DIFdelbegin \DIFdel{-factor around }\DIFdelend \DIFaddbegin \DIFadd{factor of approximately
}\DIFaddend $1.2$\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Dawson,Beenakker,Dittmaier}}%DIFAUXCMD
, the }\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Dawson,Beenakker,Dittmaier}}%DIFAUXCMD
, we find that
the expected }\DIFaddend number of
events \DIFdelbegin \DIFdel{can be raised
up }\DIFdelend \DIFaddbegin \DIFadd{increases }\DIFaddend to $\sim 5.49 \times 10^4$.
\DIFaddbegin {\bf \DIFadd{(KK: should we use 5.5 at this point, since
  the $K$ factor is only two sig figs?)}}
\DIFaddend On the other hand, additional cuts\DIFaddbegin \DIFadd{, }\DIFaddend as
well as \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a reduction in }\DIFaddend efficiency related to \DIFdelbegin \DIFdel{the reconstructionof particles
momenta will contribute to decrease }\DIFdelend \DIFaddbegin \DIFadd{momentum reconstruction,
will lead to a decrease in }\DIFaddend this number.  \DIFdelbegin \DIFdel{For instance, in
order to obtain the asymmetry of $\epsilon_4$,
}\DIFdelend \DIFaddbegin \DIFadd{In
order to measure the asymmetry $\mathcal{A}(\epsilon_4)$,
for example, }\DIFaddend the $t$ and $\tbar$
momenta need to be reconstructed. This is challenging\DIFaddbegin \DIFadd{, }\DIFaddend not only due to
the presence of two neutrinos in the final state \DIFaddbegin \DIFadd{(}\DIFaddend which escape the
detector undetected\DIFaddbegin \DIFadd{), }\DIFaddend but also because final state objects need to be
\DIFdelbegin \DIFdel{asocciated with the }\DIFdelend \DIFaddbegin \DIFadd{associated with their }\DIFaddend respective parent quark\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{atlasconf}}%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{atlasconf}}%DIFAUXCMD
.
}{\bf \DIFadd{(KK: maybe we can talk about this for a minute; I don't think I
  understand it\ldots.  Wait -- do we mean that the quarks and leptons
  need to be associated with either the top or antitop?  If so, maybe
  we can reword this a bit.  Suggestion follows\ldots, maybe this
  is too explicit now, because we also wanted it to refer to the Higgs.)}}
\DIFadd{but also because the (visible) quarks and charged leptons in the final
state need to be correctly associated with the corresponding
parent particle (i.e., the top or antitop quark)~\mbox{%DIFAUXCMD
\cite{atlasconf}}%DIFAUXCMD
.
}\DIFaddend As was
already \DIFdelbegin \DIFdel{mentioned }\DIFdelend \DIFaddbegin \DIFadd{noted }\DIFaddend in Sec.~\ref{sec5}, \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{one }\DIFaddend possibility is to use the
...
vertices involved in the process. \DIFaddbegin {\bf \DIFadd{(KK: Can we either simplify the
  previous sentence or split it
  into 2 or 3 sentences?  I'm not 100\% clear on which ideas go
  together, so I wonder if one of you could give it a try.  My preference
  would be to simplify it to something like ``\ldots weighting technique,
  along with associated kinematic constraints'' or something like
  that, and then give a reference.)}}
\DIFaddend Within the context of $t\tbar$ \DIFaddbegin \DIFadd{production }{\bf \DIFadd{(KK: correct?)}} \DIFaddend this
procedure has been used, for instance, \DIFdelbegin \DIFdel{in order to obtain 
measurements
of spin correlation}\DIFdelend \DIFaddbegin \DIFadd{to obtain 
spin correlation~}\DIFaddend \cite{atlasconf} and charge
asymmetry\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{atlascharge}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{atlascharge} }%DIFAUXCMD
measurements}\DIFaddend .
Also, events reconstructed \DIFdelbegin \DIFdel{with this technique has
}\DIFdelend \DIFaddbegin \DIFadd{using this technique have
}\DIFaddend been used in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{dosSantos} }%DIFAUXCMD
for analyzing }\DIFdelend \DIFaddbegin \DIFadd{the analysis of }\DIFaddend angular distributions that
are useful for discriminating the signal from the backgrounds in
$t\tbar H (H\rightarrow b\bbar)$\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{dosSantos}}%DIFAUXCMD
.
}{\bf \DIFadd{(KK: do we need an initial state
  on this process?)}} \DIFaddend In all these cases the corresponding
efficiency in the reconstruction of the momenta is up to \DIFdelbegin \DIFdel{$\sim 80
\%$.
}%DIFDELCMD < \par %%%
\DIFdel{Based on the discussion given }\DIFdelend \DIFaddbegin \DIFadd{$\sim 80\%$.
}{\bf \DIFadd{(KK: Does this mean that none of them exceeds 80\% or that
  they all have an efficiency of at least 80\%?)}}


\DIFadd{Given the discussion }\DIFaddend in the previous paragraph, we
have \DIFdelbegin \DIFdel{simulated }\DIFdelend \DIFaddbegin \DIFadd{generated }\DIFaddend sets of $5\times 10^4, 1\times 10^4$ and $5\times 10^3$
events and \DIFaddbegin \DIFadd{have }\DIFaddend recalculated the most sensitive observable, 
\DIFdelbegin \DIFdel{namely
}\DIFdelend $\mathcal{A}(\epsilon_4)$, for each case. The results are displayed in
Table \ref{table11}, where it can be seen that for \DIFdelbegin \DIFdel{a number of events
close to that roughly estimated above within the
context of }\DIFdelend \DIFaddbegin \DIFadd{$5\times 10^4$ events
(which is close to our rough estimate above for the
total number of signal events for }\DIFaddend the
HL-LHC\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{), }{\bf \DIFadd{(KK: stated correctly?)}}
\DIFaddend the observable is still \DIFaddbegin \DIFadd{very }\DIFaddend sensitive to $\kpt$\DIFdelbegin \DIFdel{allowing to
separate }\DIFdelend \DIFaddbegin \DIFadd{.
In this case, }\DIFaddend the $\mathrm{CP}$-mixed \DIFdelbegin \DIFdel{cases }\DIFdelend \DIFaddbegin \DIFadd{scenarios are
effectively separated }\DIFaddend by $19\sigma$. As expected, the
sensitivity \DIFdelbegin \DIFdel{worsen }\DIFdelend \DIFaddbegin \DIFadd{worsens }\DIFaddend as the number of events is reduced, but even with
$5\times 10^3$ events the separation between the $\mathrm{CP}$-mixed
\DIFdelbegin \DIFdel{hypotheses }\DIFdelend \DIFaddbegin \DIFadd{scenarios }\DIFaddend under consideration is \DIFdelbegin \DIFdel{around $6.5\sigma$}\DIFdelend \DIFaddbegin \DIFadd{$\sim 6.5\sigma$}\DIFaddend . \par
\DIFdelbegin \DIFdel{Although
the combination $\epsilon_{10}$ discussed in }\DIFdelend \DIFaddbegin 

\DIFadd{In }\DIFaddend Sec.~\ref{sec5} \DIFdelbegin \DIFdel{avoids
the problem of reconstructing the top }\DIFdelend \DIFaddbegin \DIFadd{we defined the TP combination $\epsilon_{10}$,
which does not depend directly on the top or antitop momenta.
Although the top }\DIFaddend and antitop momenta \DIFdelbegin \DIFdel{, we have
also considered the respective asymmetry obtained }\DIFdelend \DIFaddbegin \DIFadd{would not need to be
reconstructed to measure $\mathcal{A}(\epsilon_{10})$,
it is still useful to examine this observable }\DIFaddend for more
conservative numbers of events. \DIFdelbegin \DIFdel{In Table\ref{table12} we show the
results for $\mathcal{A}(\epsilon_{10})$ when the number of events is
reduced from }\DIFdelend \DIFaddbegin \DIFadd{Table~\ref{table12} shows the
results obtained for 
}\DIFaddend $5\times 10^4$ \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend $1\times 10^4$ \DIFaddbegin \DIFadd{events}\DIFaddend . We see in this case
that even with $1\times 10^4$ \DIFdelbegin \DIFdel{simulated }\DIFdelend events the observable is
\DIFdelbegin \DIFdel{capable to discriminate }\DIFdelend \DIFaddbegin \DIFadd{able to distinguish }\DIFaddend the $\mathrm{CP}$-mixed cases by
$5.6\sigma$.\par
\DIFaddbegin 

\DIFaddend Finally, it is important to mention that a realistic
analysis of the sensistivity of the observables discussed in this
paper requires \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend study of the impact of the backgrounds\DIFaddbegin \DIFadd{, }\DIFaddend as well as
the hadronization of the quarks in the final state and the effects of
the detector. If we consider the dominant decay mode of the \DIFdelbegin \DIFdel{higgs}\DIFdelend \DIFaddbegin \DIFadd{Higgs boson}\DIFaddend ,
...
missing energy\DIFdelbegin \DIFdel{, while the main background arise }\DIFdelend \DIFaddbegin \DIFadd{.  The main background arises }\DIFaddend from the production of
$t\tbar$ in \DIFdelbegin \DIFdel{asocciation }\DIFdelend \DIFaddbegin \DIFadd{association }\DIFaddend with additional jets, \DIFdelbegin \DIFdel{being }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend the dominant
source \DIFaddbegin \DIFadd{being }\DIFaddend the production of $t\tbar + b\bbar$.
\DIFdelbegin \DIFdel{By applying }\DIFdelend \DIFaddbegin \DIFadd{In Ref.~\mbox{%DIFAUXCMD
\cite{chinos} }%DIFAUXCMD
it is shown that the application of }\DIFaddend a small
set of cuts \DIFdelbegin \DIFdel{it is shown in \mbox{%DIFAUXCMD
\cite{chinos} }%DIFAUXCMD
that the
}%DIFDELCMD < \begin{table}[H]
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{results in a large improvement in the signal to background
ratio.  }{\bf \DIFaddFL{(KK: please check that I didn't say this incorrectly.)}}
\DIFaddFL{On the experimental side, a
rigorous treatment of the signal and backgrounds
for the process }{\bf \DIFaddFL{(KK: \ldots process\ldots)}}
\DIFaddFL{is performed in Ref.~\mbox{%DIFAUXCMD
\cite{atlasger}}%DIFAUXCMD
, using $20.3\,
\mathrm{fb}^{-1}$ of data at $\sqrt{s}=8\,\mathrm{TeV}$.
       }{\bf \DIFaddFL{(KK: please check that I didn't alter the meaning
         of this last sentence when I edited it.  Also, do we need
         to say anything about this study other than that it was done?)}}
...
\begin{table}[th]
\DIFaddendFL \caption{Asymmetry for the TP $\epsilon_{10}$ in the SM case and the
  two $\mathrm{CP}$-mixed cases \DIFdelbeginFL \DIFdelFL{defined by $\kp=1,\kpt=\pm 1$ when the
  number of simulated events is reduced from }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for
  }\DIFaddendFL $5\times 10^4$ \DIFdelbeginFL \DIFdelFL{to $1
  \times 10^4$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and $1\times 10^4$ events}\DIFaddendFL .}
\label{table12}
\begin{center}
...
%DIFDELCMD < %%%
\DIFdelend %
\DIFdelbegin \DIFdel{signal to background ratio is largely improved. On the other hand, a
rigorous treatment of the backgrounds and their impact with respect to
the signal is performed in \mbox{%DIFAUXCMD
\cite{atlasger} }%DIFAUXCMD
by using $20.3\,
\mathrm{fb}^{-1}$ of data at $\sqrt{s}=8\,\mathrm{TeV}$.  }%DIFDELCMD < \par %%%
\DIFdelend \DIFaddbegin 


\DIFaddend The
results shown in \DIFdelbegin \DIFdel{tables }\DIFdelend \DIFaddbegin \DIFadd{Tables }\DIFaddend \ref{table11} and \ref{table12} reveal that
with $5\times 10^3$ and $1\times 10^4$ events\DIFdelbegin \DIFdel{respectively }\DIFdelend \DIFaddbegin \DIFadd{, respectively, }\DIFaddend the
observables $\mathcal{A}(\epsilon_4)$ and $\mathcal{A}(\epsilon_{10})$
are still useful for testing $\kpt$. Without \DIFdelbegin \DIFdel{considering the lack of events related to the }\DIFdelend \DIFaddbegin \DIFadd{taking into account
the loss of events that would take place in a realistic 
}\DIFaddend experimental analysis, these numbers of events
correspond to a luminosity \DIFdelbegin \DIFdel{around }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend $\sim
300\,$-$\,600\,\mathrm{fb}^{-1}$ for the SM and even smaller for the
$\mathrm{CP}$-mixed cases \DIFdelbegin \DIFdel{due to the }\DIFdelend \DIFaddbegin \DIFadd{(due to their }\DIFaddend larger cross section\DIFaddbegin \DIFadd{)}\DIFaddend . This range
...
detector resolution, reconstruction \DIFdelbegin \DIFdel{eficciencies }\DIFdelend \DIFaddbegin \DIFadd{efficiencies }\DIFaddend and so
forth. \DIFdelbegin \DIFdel{However, this kind of analysisis out of }\DIFdelend \DIFaddbegin \DIFadd{Such an analysis, however, is beyond }\DIFaddend the scope of this
paper.
%%%%%%%%%%%% with Cuts
%51.46/17 -0.02 +- 0.08
%45.48/17  -1.09 +- 0.07
