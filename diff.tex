...
%DIF DEL ../manuscript_older/Paper_ttbarH_Draft_2_27_16.tex   Sat Feb 27 15:49:11 2016
%DIF ADD Paper_ttbarH_Draft.tex                               Tue Mar  1 22:16:09 2016
\pdfoutput=1
\usepackage{graphicx}
%\usepackage{dcolumn}
\usepackage{amsfonts}
...
and $\kpt\neq 0$ parameterize a $\mathrm{CP}$-odd Higgs \DIFdelbegin %DIFDELCMD < {\bf %%%
\DIFdel{KK: ``boson''? I don't think PRD allows us to just say ``Higgs''
}%DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{boson}\DIFaddend .
\DIFaddbegin 

\DIFadd{Before turning to a discussion of $\mathrm{CP}$-odd observables,
it is useful to consider a few theoretical aspects of
...
that will be useful as we construct $\mathrm{CP}$-odd observables.}\DIFaddend \par
\DIFaddbegin 
...
\end{center}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFdelbegin \DIFdel{In this section }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Factorized expression for the scattering cross section}}
\label{subsec:factorize}

\DIFadd{In this subsection }\DIFaddend we focus on the gluon fusion
\DIFaddbegin {\bf \DIFadd{(KK: seems repetitive to say ``gluon fusion'' right away
...
diagrams indicated in Fig.~\ref{fig1}\DIFdelbegin %DIFDELCMD < {\bf %%%
\DIFdel{(KK: correct?)}%DIFDELCMD < }%%%
\DIFdelend ; also,
...
bosons.\footnote{Since Eq.~(\ref{eq9}) contains the top quark propagator
  ...
  in the narrow-width approximation.  Thus\DIFaddbegin \DIFadd{, except for the propagator terms $\mathbb{P}_t(t)$ and $\mathbb{P}_t(\tbar)$,
   }\DIFaddend we take the four-vector $t$ appearing in \DIFdelbegin \DIFdel{the definition of $\phi_t$ }\DIFdelend \DIFaddbegin \DIFadd{Eqs.~(\ref{eq9})-(\ref{eq11}) }\DIFaddend to be
  on shell, satisfying $t^2 = m_t^2$.} 
\DIFdelbegin %DIFDELCMD < {\bf %%%
\DIFdel{(KK: Do I understand correctly
  that $t$ is an on-shell four-vector in $t\cdot \ell^+$, but not
  in $\mathbb{P}_t(t)$?  This is similar to saying $f(x)\delta(x-x_0) =
  f(x_0)\delta(x-x_0)$\ldots?)}%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend Working out the projection operators $\phi_t\,\bar{\phi}_t$
...
\DIFdel{(KK: I'm still a bit confused as to why $n_t$ appears
  ...
  Eq.~(17).  Probably need to look more closely at the notes!)}%DIFDELCMD < }
%DIFDELCMD <  %%%
\DIFdelend 
...
\subsection{\DIFdelbegin \DIFdel{Triple products}\DIFdelend \DIFaddbegin \DIFadd{Origin of triple product terms}\DIFaddend }
\DIFdelbegin %DIFDELCMD < {\bf %%%
\DIFdel{(KK: maybe we should change this title for the subsection\ldots.)}%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{subsec:origin}
\DIFaddend 
...
\DIFdelbegin %DIFDELCMD < {\bf %%%
\DIFdel{(KK: do these need $k$ subscripts?)}%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdel{$\mathcal{S}^{ab,ij}= C^{ab,ij}_k \mathcal{S}$ and
$\mathcal{P}^{ab,ij}= C^{ab,ij}_k \mathcal{P}$
}\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{S}^{\,ab,ij}_k= C^{ab,ij}_k \mathcal{S}_k$ and
$\mathcal{P}^{\,ab,ij}_k= C^{ab,ij}_k \mathcal{P}_k$
}\DIFaddend [see Eqs.~(\ref{eq5}) and (\ref{eq16})]. Also, the factors
...
C^{ab,ij*}_r$ is real and where \DIFdelbegin \DIFdel{$\tilde{\mathcal{P}}_r = \gamma^0
\mathcal{P}_r \gamma^0$ }%DIFDELCMD < {\bf %%%
\DIFdel{(KK: wouldn't this be
  $\mathcal{P}_r^\dagger$? I didn't look for this in the notes
  yet, so maybe I'm just missing something)}%DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{$\tilde{\mathcal{P}}_r = \gamma^0
\mathcal{P}^{\dagger}_r \gamma^0$}\DIFaddend .
...
\DIFdelbegin %DIFDELCMD < {\bf %%%
\DIFdel{(KK: I may not have worded this quite correctly.)}%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend 
...
followed by Higgs decay to two photons }%DIFDELCMD < {\bf %%%
...
resolve the degeneracy \DIFdelbegin %DIFDELCMD < {\bf %%%
\DIFdel{(KK: did I reword this correctly?)}%DIFDELCMD < }
%DIFDELCMD < %%%
...
\DIFaddend The first type of $\mathrm{CP}$-odd observable \DIFaddbegin \DIFadd{that }\DIFaddend we will consider is \DIFdelbegin \DIFdel{the
asymmetry. For a certain TP, we define an
asymmetry which gives the balance between the }\DIFdelend \DIFaddbegin \DIFadd{an
asymmetry that compares the }\DIFaddend number of events
\DIFdelbegin \DIFdel{with $\epsilon > 0$ and those for which $\epsilon < 0$:
}\DIFdelend \DIFaddbegin \DIFadd{for which a given TP is positive to that for which it is negative.
...
\eeq 
%
\DIFaddbegin \DIFadd{By construction, $\mathcal{A}\in [-1,+1]$.
}\DIFaddend Based on the general expression given in Eq.~(\ref{eq20}), we expect
the following \DIFdelbegin \DIFdel{shape }\DIFdelend \DIFaddbegin \DIFadd{functional form }\DIFaddend for the asymmetry,
%
\beq
\label{eq22}
...
from \DIFdelbegin \DIFdel{the }\DIFdelend linear behaviour.
\DIFdelbegin \DIFdel{The results for }\DIFdelend \DIFaddbegin 

\DIFadd{Table~\ref{table1} shows numerical results for the
asymmetries associated with three different TPs,
$\epsilon_1$, $\epsilon_2$ and $\epsilon_3$, taking
}\DIFaddend $\kp=1$ and $\kpt=0,\pm 1$\DIFdelbegin \DIFdel{are shown in Table \ref{table1}, where the values in terms of the
}\DIFdelend \DIFaddbegin \DIFadd{.  The asymmetry $\mathcal{A}$
is shown in each case, along with $\mathcal{A}/\sigma_{\mathcal{A}}$, where
$\sigma_{\mathcal{A}}$ is the }\DIFaddend corresponding
statistical uncertainty\DIFdelbegin \DIFdel{are also included.  It can be
seen that the considered asymmetries are useful to separate the }\DIFdelend \DIFaddbegin \DIFadd{.  As is
evident from the table, the asymmetries in question provide
a clear separation between the SM and the 
}\DIFaddend $\mathrm{CP}$-mixed cases\DIFdelbegin \DIFdel{from the SM case and also to discriminate
the different }\DIFdelend \DIFaddbegin \DIFadd{, with typical deviations being of
...
determine the }\DIFaddend sign of $\kpt$,
\DIFdelbegin \DIFdel{giving deviations of order $10\sigma$ in
the former case and }\DIFdelend \DIFaddbegin \DIFadd{with deviations between the $\kpt = \pm 1$ cases typically
being of order }\DIFaddend $20\sigma$\DIFdelbegin \DIFdel{in the latter.
}%DIFDELCMD < \par %%%
\DIFdelend \DIFaddbegin \DIFadd{.
}{\bf \DIFadd{(We couldn't figure out how to clarify this. If you find a way go ahead\ldots.)}}
\DIFaddend The sensitivity of
...
Eq.~(\ref{eq23}) as \DIFaddbegin \DIFadd{a }\DIFaddend fitting function (see Fig.~\ref{fig3}).
\DIFdelbegin \DIFdel{In fact}\DIFdelend \DIFaddbegin \DIFadd{Performing such a fit}\DIFaddend , 
we obtain \DIFdelbegin \DIFdel{the fit parameters }\DIFdelend $(a=-0.057\pm 0.006, b=0.5\pm 0.2),
(a=-0.056\pm 0.006, b=0.5 \pm 0.2)$ and $(a=0.058\pm 0.006, b=0.6 \pm
0.2)$ for $\epsilon_1$, $\epsilon_2$ and $\epsilon_3$,
respectively.
\DIFdelbegin \DIFdel{In addition, we include in }\DIFdelend \DIFaddbegin 

\DIFadd{The results shown in Table~\ref{table1}
and }\DIFaddend Fig.~\DIFdelbegin \DIFdel{\ref{fig4} the asymmetry
for }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig3} all assume a $pp$ initial state, which is
...
three curves for the ``}\DIFaddend $\epsilon_1$\DIFdelbegin \DIFdel{isolating the }\DIFdelend \DIFaddbegin \DIFadd{'' case, one for }\DIFaddend $gg$-initiated
\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{events, one for }\DIFaddend $q\qbar$-initiated \DIFdelbegin \DIFdel{and
}\DIFdelend \DIFaddbegin \DIFadd{events, and one for the usual
combination of these events (the ``}\DIFaddend $pp$\DIFdelbegin \DIFdel{production.
We see }\DIFdelend \DIFaddbegin \DIFadd{'' initial state).
Interestingly, we see from Fig.~\ref{fig4} }\DIFaddend that the asymmetry \DIFaddbegin \DIFadd{for this TP
}\DIFaddend is enhanced for
$gg$-initiated production, while it is reduced and of opposite sign
for the $q\qbar$-initiated \DIFdelbegin \DIFdel{case}\DIFdelend \DIFaddbegin \DIFadd{events}\DIFaddend . The asymmetry for the $pp$ case is
\DIFdelbegin \DIFdel{then }\DIFdelend \DIFaddbegin \DIFadd{evidently
}\DIFaddend dominated by the $gg$ contribution, \DIFdelbegin \DIFdel{albeit smaller
}\DIFdelend \DIFaddbegin \DIFadd{but is somewhat smaller
in magnitude }\DIFaddend due to the
$q\qbar$ contribution.
\newcolumntype{C}[1]{>{\centering\arraybackslash}p{#1}}
\renewcommand{\arraystretch}{1.4}
\begin{table}[H]
  \caption{Asymmetries \DIFaddbeginFL \DIFaddFL{for three different scenarios,
    }\DIFaddendFL obtained by using $10^5$ simulated events for the
    TPs $\epsilon_1=\TPa, \epsilon_2=\TPb$ and $\epsilon_3=\TPc$\DIFdelbeginFL \DIFdelFL{in }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{.
    The three scenarios
  correspond to }\DIFaddendFL the SM \DIFdelbeginFL \DIFdelFL{case }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{($\kp=1$ }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\kpt=\pm 0$) and
  }\DIFaddendFL two $\mathrm{CP}$-mixed cases \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL defined by
  \DIFdelbeginFL \DIFdelFL{$\kp=1,\kpt=\pm 1$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\kp=1$ and $\kpt=\pm 1$)}\DIFaddendFL .}
\label{table1}
\begin{center}
...
\DIFadd{We }\DIFaddend have also tested various
linear combinations of the TPs $\epsilon_{1,2,3}$ \DIFdelbegin \DIFdel{, with the result
}\DIFdelend \DIFaddbegin \DIFadd{and have found
}\DIFaddend that the asymmetry is enhanced \DIFdelbegin \DIFdel{in the following case}\DIFdelend \DIFaddbegin \DIFadd{for the following combination}\DIFaddend :
%
\beq
...
\eeq
%
Note that in the $Q$ rest frame\DIFaddbegin \DIFadd{, }\DIFaddend $\epsilon_4=Q^0
...
\DIFaddbegin {\bf \DIFadd{(KK: try to connect previous sentences to something later on\ldots.)}}
\DIFaddend The
values obtained for the asymmetry \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{associated with }\DIFaddend this TP are shown in
\DIFdelbegin \DIFdel{\ref{table2}. We see by }\DIFdelend \DIFaddbegin \DIFadd{Table~\ref{table2}. By }\DIFaddend comparing the results in Tables \ref{table1}
and \ref{table2}\DIFaddbegin \DIFadd{, we see }\DIFaddend that the capability of \DIFdelbegin \DIFdel{the asymmetry to separate }\DIFdelend \DIFaddbegin \DIFadd{this
asymmetry to distinguish between }\DIFaddend the
...
\end{center} 
\end{table}
\par Finally, it is worth noting that the \DIFdelbegin \DIFdel{asymmetry is }\DIFdelend \DIFaddbegin \DIFadd{asymmetries
described in this subsection are }\DIFaddend not useful for
discriminating between the SM hypothesis ($\kp=1,\kpt=0$) and the pure
pseudoscalar hypothesis ($\kp=0,\kpt=1$)\DIFdelbegin \DIFdel{because, being }\DIFdelend \DIFaddbegin \DIFadd{.  Since
the numerators of the 
asymmetries are all }\DIFaddend linear in both
$\kp$ and $\kpt$, \DIFdelbegin \DIFdel{it is }\DIFdelend \DIFaddbegin \DIFadd{they are }\DIFaddend expected to vanish in these cases. However, we
will show in the next \DIFdelbegin \DIFdel{section }\DIFdelend \DIFaddbegin \DIFadd{subsection
}\DIFaddend that there exist angular distributions
derived from the TPs that are actually suitable \DIFdelbegin \DIFdel{to distinguish these
two hyphothesis}\DIFdelend \DIFaddbegin \DIFadd{for distinguishing between these
two hyphotheses}\DIFaddend .
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Angular Distributions}
\label{sec3.2}
\DIFaddbegin 

\DIFaddend Given a certain TP, it is possible to define \DIFdelbegin \DIFdel{asocciated }\DIFdelend \DIFaddbegin \DIFadd{associated }\DIFaddend angular
distributions that are \DIFdelbegin \DIFdel{also }\DIFdelend sensitive to the pseudoscalar coupling
$\kpt$. In order to clarify this, let us first consider the TP
$\epsilon(t,\tbar,n_t,n_{\tbar})$. This TP can be written as
$\epsilon(t+\tbar,\tbar,n_t,n_{\tbar})$\DIFaddbegin \DIFadd{, }\DIFaddend so that in the reference frame
...
of $\vec{n}_t$ and $\vec{n}_{\tbar}$\DIFdelbegin \DIFdel{respectively and finally
}\DIFdelend \DIFaddbegin \DIFadd{, respectively, and 
}\DIFaddend $\Delta\phi(n_t,n_{\tbar})$ is the angular difference between the
projections of $\vec{n}_t$ and $\vec{n}_{\tbar}$ onto the plane
perpendicular to $\vec{\bar{t}}$. If we define \DIFdelbegin \DIFdel{this angle
}\DIFdelend \DIFaddbegin \DIFadd{the angle
$\Delta\phi(n_t,n_{\tbar})$  to be }\DIFaddend within the
range $[-\pi,\pi]$, we see from Eq.~(\ref{eq25}) that its sign will
determine the sign of the TP. \DIFdelbegin \DIFdel{In this sense}\DIFdelend \DIFaddbegin \DIFadd{Thus}\DIFaddend , the distribution of the
number of \DIFdelbegin \DIFdel{the }\DIFdelend events with respect to the angle
...
\DIFdelbegin \DIFdel{Moreover, from }\DIFdelend \DIFaddbegin {\bf \DIFadd{(KK: Have we checked numerically that the above
  expression does indeed hold numerically for the various
  distributions?  It's an exact relation, correct?)}}
\DIFadd{Moreover, for }\DIFaddend a certain TP
one can derive different angular distributions by considering
different reference frames, \DIFdelbegin \DIFdel{but }\DIFdelend \DIFaddbegin \DIFadd{although }\DIFaddend all of these will satisfy
Eq.~(\ref{eq26}) \DIFdelbegin \DIFdel{. In particular, we will analyze the following angular
distributions based on the TPs defined }\DIFdelend \DIFaddbegin \DIFadd{(note that $\mathcal{A}(\epsilon)$
is Lorentz invariant). Recalling the various TPs considered
}\DIFaddend in Sec.~\ref{sec2}\DIFdelbegin \DIFdel{:
}%DIFDELCMD < \begin{itemize}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{, we examine the following angular distributions.
} \begin{enumerate} 
\DIFaddend \item \DIFdelbegin \DIFdel{Related to }\DIFdelend \DIFaddbegin {\boldmath \DIFaddend $\epsilon_1 = \TPa$\DIFdelbegin \DIFdel{we consider
  }\DIFdelend \DIFaddbegin \DIFadd{.}}  \DIFadd{To probe $\epsilon_1$, we construct
  the distribution
  }\DIFaddend $d\sigma/d\Delta\phi_1(n_t,n_{\tbar})$ in the rest frame of $t\tbar$\DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{,
  taking }\DIFaddend $\vec{\tbar}$ \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{to define }\DIFaddend the $z$-axis. The angle
  $\Delta\phi_1(n_t,n_{\tbar})$ is the angular difference between the
  projection of the spin vectors in the plane perpendicular to
  $\vec{\tbar}$.
\item \DIFdelbegin \DIFdel{Related to }\DIFdelend \DIFaddbegin {\boldmath \DIFaddend $\epsilon_2 = \TPb$\DIFdelbegin \DIFdel{we consider
  }\DIFdelend \DIFaddbegin \DIFadd{.}}  \DIFadd{In this case, we
  define the distribution
  }\DIFaddend $d\sigma/d\Delta\phi_2(n_t,n_{\tbar})$ in the rest frame of $Q$\DIFdelbegin \DIFdel{with
  }\DIFdelend \DIFaddbegin \DIFadd{, taking
  }\DIFaddend $\vec{\tbar}$ \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{to define }\DIFaddend the $z$-axis. The angle
  $\Delta\phi_2(n_t,n_{\tbar})$ is the angular difference between the
  projection of the spin vectors in the plane perpendicular to
  $\vec{\tbar}$.
\item \DIFdelbegin \DIFdel{Related to }\DIFdelend \DIFaddbegin {\boldmath \DIFaddend $\epsilon_3 = \TPc$\DIFdelbegin \DIFdel{we consider
  }\DIFdelend \DIFaddbegin \DIFadd{.}}  \DIFadd{The distribution
  }\DIFaddend $d\sigma/d\Delta\phi_3(n_t,n_{\tbar})$ \DIFaddbegin \DIFadd{is also defined
  }\DIFaddend in the rest frame of $Q$\DIFdelbegin \DIFdel{with
  }\DIFdelend \DIFaddbegin \DIFadd{, but this time taking
  }\DIFaddend $\vec{t}$ \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{to be along }\DIFaddend the $z$-axis. The angle $\Delta\phi_3(n_t,n_{\tbar})$
...
\DIFaddend \par
%
\DIFdelbegin \DIFdel{In Fig.~\ref{fig5} we present }\DIFdelend \DIFaddbegin \DIFadd{Figure~\ref{fig5} shows }\DIFaddend the normalized distributions obtained
for the first case listed above\DIFdelbegin \DIFdel{with $\kp= 1,\kpt=0$
(}\DIFdelend \DIFaddbegin \DIFadd{.
Four scenarios are considered, corresponding to the SM
($\kp= 1$ and $\kpt=0$), two cases in which the Higgs boson
has mixed }\DIFaddend $\mathrm{CP}$ \DIFdelbegin \DIFdel{-even Higgs boson), $\kp= 1,\kpt=\pm 1$
($\mathrm{CP}$-mixed Higgs boson
) }\DIFdelend \DIFaddbegin \DIFadd{couplings ($\kp= 1$ }\DIFaddend and \DIFdelbegin \DIFdel{$\kp= 0,\kpt=1$
(}\DIFdelend \DIFaddbegin \DIFadd{$\kpt=\pm 1$)
and a case in which the Higgs boson is purely 
}\DIFaddend $\mathrm{CP}$\DIFdelbegin \DIFdel{-odd Higgs boson ). In Fig.~\ref{fig6} we display in turn
}\DIFdelend \DIFaddbegin \DIFadd{-odd ($\kp= 0,\kpt=1$). Figure~\ref{fig6} shows
}\DIFaddend the analogous distributions for $\epsilon_2$. The distributions
\DIFdelbegin \DIFdel{correponding }\DIFdelend \DIFaddbegin \DIFadd{corresponding }\DIFaddend to $\epsilon_3$ are similar to those of $\epsilon_2$\DIFdelbegin \DIFdel{but
with the opposite shift for
$\kp=\kpt=1$ and $\kp=-\kpt=1$ and then }\DIFdelend \DIFaddbegin \DIFadd{,
except that the ``shifts'' are in the opposite directions for
the two mixed-$\mathrm{CP}$ cases.  Given the similarities of the
plots }\DIFaddend we do not include them here.  \DIFaddbegin {\bf \DIFadd{(KK: did I reword this correctly?)}}
\DIFaddend %%%%%%%%%%%%%%%%%%%%%%%%% FIGURA 5 %%%%%%%%%%%%%%%%%%%%%%%%%
...
\caption{Angular distributions associated with the TP
  \DIFdelbeginFL \DIFdelFL{$\epsilon(t,\tbar,n_t,n_{\tbar})$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\epsilon_1 = \epsilon(t,\tbar,n_t,n_{\tbar})$ }\DIFaddendFL for various values of
  \DIFdelbeginFL \DIFdelFL{$(\kp,\kpt)$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\kp$ and $\kpt$}\DIFaddendFL . The error bars correspond to the statistical
  uncertainties.}
\label{fig5}
\end{figure}
\end{center}
...
\caption{Angular distributions associated with the TP \DIFdelbeginFL \DIFdelFL{$\TPb$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\epsilon_2 = \TPb$ }\DIFaddendFL for
  various values of \DIFdelbeginFL \DIFdelFL{$(\kp,\kpt)$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\kp$ and $\kpt$}\DIFaddendFL . The error bars \DIFdelbeginFL \DIFdelFL{correspond to }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{indicate }\DIFaddendFL the statistical
  uncertainties.}
\label{fig6}
\end{figure}
\end{center}
\par As can be seen from Figs.~\ref{fig5} and \ref{fig6}, the
\DIFdelbegin \DIFdel{peak }\DIFdelend \DIFaddbegin \DIFadd{peaks }\DIFaddend of the 
distributions \DIFdelbegin \DIFdel{is shifted according to the values of the couplings
$\kp, \kpt$.
This }\DIFdelend \DIFaddbegin \DIFadd{are shifted to the left or the right of the origin
in the mixed-$\mathrm{CP}$
cases ($\kp =1$ and $\kpt=\pm 1$).
The magnitude of the }\DIFaddend shift appears to be approximately the
same \DIFaddbegin \DIFadd{in both cases, }\DIFaddend but with
opposite sign for $\kp=\kpt=1$ and $\kp=-\kpt=1$, \DIFdelbegin \DIFdel{distinguishing then
}\DIFdelend \DIFaddbegin \DIFadd{thus allowing
one to distinguish
}\DIFaddend the sign of the pseudoscalar coupling. The \DIFdelbegin \DIFdel{last is the expected
behaviour since the asymmetry }\DIFdelend \DIFaddbegin \DIFadd{observed dependence
on the sign of $\kpt$ in these cases
is consistent with the fact that the numerator of $\mathcal{A}(\epsilon)$
}\DIFaddend is linear in $\kpt$ \DIFdelbegin \DIFdel{(see Sec.~\ref{sec3.1})and }\DIFdelend \DIFaddbegin [\DIFadd{see Eq.~(\ref{eq26})}]
\DIFadd{and that }\DIFaddend the quantity $N(\epsilon < 0)/N_T$ is related
to the angular distribution according to Eq.~(\ref{eq26}). The \DIFaddbegin \DIFadd{angular
}\DIFaddend distributions for the SM \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{case ($\kp =1$ and $\kpt=0$) and
}\DIFaddend the pure pseudoscalar \DIFdelbegin \DIFdel{hypotheses are
clearly different . While in the former case the distributions }\DIFdelend \DIFaddbegin \DIFadd{case ($\kp =0$ and $\kpt= 1$) are
...
}\DIFaddend exhibit a minimum at $\Delta\phi_{1,2}(n_t,n_{\tbar})=0$, \DIFdelbegin \DIFdel{in the latter the
similar distributions display a peak , so that the two hypotheses can be discriminated, in contrast to the asymmetry that vanishes for both
hypotheses}\DIFdelend \DIFaddbegin \DIFadd{those
...
in both cases}\DIFaddend .  In order to quantify the mentioned shifts, we have fitted
