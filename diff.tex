...
%DIF DEL ../manuscript_older/Paper_ttbarH_Draft_3_9_16.tex   Wed Mar  9 13:36:44 2016
%DIF ADD Paper_ttbarH_Draft.tex                              Wed Mar  9 22:37:03 2016
\pdfoutput=1
\usepackage{graphicx}
%\usepackage{dcolumn}
\usepackage{amsfonts}
...
\DIFaddbegin \DIFadd{We generate Monte Carlo data sets for several benchmark scenarios,
including the Standard Model ($\kp=1$, $\kpt=0$) and
two scenarios with mixed $\mathrm{CP}$ properties ($\kp=1$, $\kpt=\pm 1$).
}{\bf \DIFadd{(KK: worded correctly?)}}
\DIFaddend Assuming an integrated luminosity that is consistent with that
envisioned for the \DIFdelbegin \DIFdel{HL-LHC}\DIFdelend \DIFaddbegin \DIFadd{High Luminosity Large Hadron Collider}\DIFaddend ,
and taking into account
only statistical uncertainties,
\DIFdelbegin %DIFDELCMD < {\bf %%%
\DIFdel{(KK: do we need to define the HL-LHC?)}%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend we find that the most
promising observable can disentangle the
\DIFdelbegin \DIFdel{scenarios $\kp=1,\kpt=\pm 1$
}\DIFdelend \DIFaddbegin \DIFadd{``$\mathrm{CP}$-mixed'' scenarios 
}\DIFaddend with an effective separation of $\sim 20\sigma$.  In
...
\DIFdelbegin \DIFdel{capability of the }\DIFdelend most promising observables \DIFdelbegin \DIFdel{for separating }\DIFdelend \DIFaddbegin \DIFadd{can still disentangle }\DIFaddend the
$\mathrm{CP}$-mixed \DIFdelbegin \DIFdel{hypotheses preveals even when a }\DIFdelend \DIFaddbegin \DIFadd{scenarios when the }\DIFaddend number of events
\DIFdelbegin \DIFdel{plausible within the short termLHC is considered}\DIFdelend \DIFaddbegin \DIFadd{is reduced to values consistent with expectations
for the Large Hadron Collider in the near term}\DIFaddend .

%DIF < We also show that the sensitivity of the most promising observables is not spoiled when a number of events affordable by the short term LHC is considered.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < We also show that even when the number of events is reduced by an order of magnitude the most promising observables are still useful for testing the couplings
...
triple product (TP) structures that we extract \DIFdelbegin \DIFdel{naturally }\DIFdelend from the
...
allows \DIFdelbegin \DIFdel{to identify straightforwardly the contributions }\DIFdelend \DIFaddbegin \DIFadd{for the straightforward identification of the contributions
that are }\DIFaddend linearly
sensitive to the couplings.
\DIFdelbegin \DIFdel{Further, the }\DIFdelend TPs correlations in these
contributions incorporate the $t$ and $\tbar$ spin vectors\DIFdelbegin \DIFdel{. Starting
}\DIFdelend \DIFaddbegin \DIFadd{; starting
}\DIFaddend with these TPs, we not only recover the observables given in
\DIFaddbegin \DIFadd{Refs.~}\DIFaddend \cite{Ellis,Guadagnoli} but also propose additional possibilities that
\DIFdelbegin \DIFdel{increase the sensitivity }\DIFdelend \DIFaddbegin \DIFadd{have an increased sensitivity to the $tH$ coupling}\DIFaddend .
In order to establish a hierarchy in the
sensitivity of the TPs under analysis we \DIFaddbegin \DIFadd{use simulated events
to }\DIFaddend investigate three different
types of observables\DIFdelbegin \DIFdel{by using simulated events}\DIFdelend : asymmetries, mean
values and angular distributions. We note that TP correlations have
been used in \DIFaddbegin \DIFadd{Ref.~}\DIFaddend \cite{Valencia1,*Valencia2} in the context of top-quark
production and decay and in \DIFaddbegin \DIFadd{Ref.~}\DIFaddend \cite{Valencia3} in the framework of
...
cross section\DIFdelbegin \DIFdel{from which a }\DIFdelend \DIFaddbegin \DIFadd{.  A }\DIFaddend first set of TP correlations is \DIFdelbegin \DIFdel{extracted}\DIFdelend \DIFaddbegin \DIFadd{then
extracted from this expression}\DIFaddend .
In Sec.~\ref{sec3} we probe the sensitivity of these TPs to
the $tH$ coupling by using various $\mathrm{CP}$-odd
observables. Subsequent sections are dedicated to \DIFdelbegin \DIFdel{explore another
possibilities of
}\DIFdelend \DIFaddbegin \DIFadd{the analysis of
other }\DIFaddend $\mathrm{CP}$-odd observables. In particular,
observables based on TPs that incorporate the Higgs momentum
\DIFaddbegin {\bf \DIFadd{(KK: not all of them involve the Higgs momentum in this section\ldots
  is this the best way to describe this section?
  I'm fine with it, either way.)}} \DIFaddend are
discussed in Sec.~\ref{sec4}, \DIFdelbegin \DIFdel{whereas observables obtained without
using }\DIFdelend \DIFaddbegin \DIFadd{and observables that do not
involve }\DIFaddend the $t$ and $\tbar$ momenta are studied in
Sec.~\ref{sec5}.  \DIFdelbegin \DIFdel{Finally, }\DIFdelend \DIFaddbegin \DIFadd{In }\DIFaddend Sec.~\ref{sec6} \DIFdelbegin \DIFdel{is devoted to the discussion
on the }\DIFdelend \DIFaddbegin \DIFadd{we discuss
the }\DIFaddend experimental feasibility of the most promising observables
...
\caption{Schematic representation of the process $g_ag_b\to t(\to
  ...
  indices $i,j$ denote the \DIFdelbeginFL \DIFdelFL{colour }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{colours }\DIFaddendFL of the quarks\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL while $a,b$ are gluon
  indices.}
\label{fig2}
\end{figure}
\end{center}
...
\DIFaddbegin \DIFadd{We shall often focus on the scenarios with $\kp=1$ and
$\kpt=\pm 1$, which we shall refer to as the ``$\mathrm{CP}$-mixed''
scenarios.
}\DIFaddend In addition, we also provide some analysis regarding the pure
...
\begin{table}[H]
  \caption{Asymmetries for three different scenarios,
    obtained by using $10^5$ simulated events\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL for the
    TPs $\epsilon_1=\TPa, \epsilon_2=\TPb$ and $\epsilon_3=\TPc$.
    The three scenarios
  correspond to the SM ($\kp=1$ and \DIFdelbeginFL \DIFdelFL{$\kpt=\pm 0$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\kpt= 0$}\DIFaddendFL ) and \DIFaddbeginFL \DIFaddFL{the
  }\DIFaddendFL two \DIFaddbeginFL \DIFaddFL{``}\DIFaddendFL $\mathrm{CP}$-mixed\DIFaddbeginFL \DIFaddFL{'' }\DIFaddendFL cases (defined by
  $\kp=1$ and $\kpt=\pm 1$).}
\label{table1}
\begin{center}
...
\DIFdelbegin %DIFDELCMD < {\bf %%%
\DIFdel{(KK: try to connect previous sentences to something later on\ldots.)}%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend The
values obtained for the asymmetry associated with this TP are shown in
Table~\ref{table2}. By comparing the results in Tables\DIFaddbegin \DIFadd{~}\DIFaddend \ref{table1}
and \ref{table2}, we see that the capability of this
asymmetry to distinguish \DIFdelbegin \DIFdel{(}%DIFDELCMD < {\bf %%%
\DIFdel{N: Do we add ``effectively" ?}%DIFDELCMD < }%%%
\DIFdel{) }\DIFdelend between the
two $\mathrm{CP}$-mixed \DIFdelbegin \DIFdel{hypotheses }\DIFdelend \DIFaddbegin \DIFadd{scenarios }\DIFaddend is increased by at least $3\sigma$.
\DIFaddbegin {\bf \DIFadd{(KK: looks like 2.5$\sigma$ is the min difference?  maybe a rounding
  thing; but see also the comment in the table caption.)}}
\DIFaddend \vspace{4mm}
\begin{table}[H]
\caption{Asymmetry for the TP $\epsilon_{4}$ for the SM case and the
  ...
  values are obtained \DIFdelbeginFL \DIFdelFL{by }\DIFdelendFL using \DIFaddbeginFL \DIFaddFL{sets of }\DIFaddendFL $10^5$ simulated events.
\DIFaddbeginFL {\bf \DIFaddFL{(KK: In the previous table we used 3 sig figs for the ratios like ``12.0'', etc.  Should we do the same in both tables for consistency?)}}\DIFaddendFL }
\label{table2}
\begin{center}
...
Eq.~(\ref{eq26}) 
gives $\mathcal{A}(\epsilon_i)=4a_1 \sin\delta$ \DIFaddbegin {\bf \DIFadd{(KK: sign
  incorrect? numbers don't agree between Tables I and III otherwise)}}\DIFaddend . With
...
for $\epsilon_1$ and $\epsilon_2$.\footnote{The results for the
  ...
  here.}  However, the quality of the fits in the four \DIFdelbegin \DIFdel{considered scenarios }\DIFdelend \DIFaddbegin \DIFadd{scenarios considered }\DIFaddend is
...
\DIFaddbegin \DIFadd{the }\DIFaddend two $\mathrm{CP}$-mixed cases
...
sensitive than $\langle \epsilon_1 \rangle$.  \DIFdelbegin \DIFdel{On the other hand}\DIFdelend \DIFaddbegin \DIFadd{Also}\DIFaddend , the
mean value for the combination $\epsilon_4$ introduced in
Sec.~\ref{sec3.1} gives slightly smaller values than those listed in
Table\DIFdelbegin \DIFdel{\ref{table5}, }\DIFdelend \DIFaddbegin \DIFadd{~\ref{table5} for $\langle \epsilon_2
\rangle$ and $\langle \epsilon_3 \rangle$,
       }{\bf \DIFadd{(KK: explicit reference to $\epsilon_2$ and $\epsilon_3$ correct?
         Seems like $\epsilon_4$ is actually a bit better than $\epsilon_1$)}}
\DIFaddend yielding $-4.32$, $1.11$ and $7.23$ for the cases
...
We see that \DIFdelbegin \DIFdel{the TP }\DIFdelend $\epsilon_5$ gives rise to asymmetries and mean
values that are clearly higher than those obtained \DIFdelbegin \DIFdel{from }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend $\epsilon_6$
and $\epsilon_7$. This is in contrast to the TPs $\epsilon_{1,2,3}$,
for which the asymmetries and mean values are comparable \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{among }\DIFaddend the
TPs
(see Tables
...
appear to have a higher sensitivity to the pseudoscalar coupling than \DIFaddbegin \DIFadd{do
}\DIFaddend $\epsilon_{5,6,7}$.  \par

It is important to mention that in the
$t\tbar$ rest frame the sign of the TP $\epsilon_5$ is defined through
the angle \DIFdelbegin \DIFdel{$\Delta\phi_{\ell^-\ell^+}$ }\DIFdelend \DIFaddbegin \DIFadd{$\Delta\phi(\ell^-,\ell^+)$ }{\bf \DIFadd{(KK: notation change OK? making this
  similar to the others)}} \DIFaddend [see the discussion following
  Eq.~(\ref{eq25})],
which is the angular difference between the
projections of the leptons' momenta onto the plane perpendicular to
$\vec{\tbar}$.  \DIFdelbegin \DIFdel{By a similar argument to that discussed at the
beginning of }\DIFdelend \DIFaddbegin \DIFadd{As in }\DIFaddend Sec.~\ref{sec3.2}, we can construct an associated angular
distribution [see Eq.~(\ref{eq26})] that
will be sensitive to the sign of the pseudoscalar coupling. \DIFdelbegin \DIFdel{This
angular variable }\DIFdelend \DIFaddbegin \DIFadd{The
angular variable $\Delta\phi(\ell^-,\ell^+)$ }{\bf \DIFadd{(KK: OK?)}}
\DIFaddend is the same as that proposed in Ref.~\cite{Ellis} as a
...
distribution since \DIFdelbegin \DIFdel{this distribution }\DIFdelend \DIFaddbegin \DIFadd{the $\Delta\phi(\ell^-,\ell^+)$ distribution }{\bf \DIFadd{(KK: OK?)}}
\DIFaddend is constrained by the asymmetry $\mathcal{A}(\epsilon_5)$
[via Eq.~(\ref{eq26})],
which in turn is equal to
$\mathcal{A}(\epsilon_1)$. Also, we note that $\mathcal{A}(\epsilon_5)$ is slightly \DIFdelbegin \DIFdel{smaller 
}\DIFdelend \DIFaddbegin \DIFadd{less sensitive
}\DIFaddend than $\mathcal{A}(\epsilon_2)$,
as can be seen from Table~\ref{table1}.
\par

\DIFaddbegin {\bf \DIFadd{(KK: moved this paragraph, since we have already
  discussed the $\epsilon_5$ angular distribution.)}}
\DIFadd{In addition to the $\epsilon_5$ angular distribution
...
enhanced \DIFdelbegin \DIFdel{with respect to }\DIFdelend \DIFaddbegin \DIFadd{compared to those for }\DIFaddend $\epsilon_5$-$\epsilon_7$,
%
\beq
\label{eq32}
...
\vec{\ell}^-)$. Comparing Eqs.~(\ref{eq24}) and (\ref{eq32})\DIFaddbegin \DIFadd{,
}\DIFaddend and noting that $Q=(t+\tbar+H)/2$, we see that the only
...
The number of events expected at the \DIFaddbegin \DIFadd{High Luminosity Large
Hadron Collider (}\DIFaddend HL-LHC\DIFaddbegin \DIFadd{)}\DIFaddend , however, is
...
$5.6\sigma$\DIFdelbegin \DIFdel{(}%DIFDELCMD < {\bf %%%
\DIFdel{N: Do we add ``effectively" distinguish?}%DIFDELCMD < }%%%
\DIFdel{).
}%DIFDELCMD < \par
%DIFDELCMD < %%%
...
also sensitive to the \DIFdelbegin \DIFdel{value }\DIFdelend \DIFaddbegin \DIFadd{values }\DIFaddend of $\kp$ and $\kpt$, exhibiting a phase
...
by at least $3\sigma$. \DIFaddbegin {\bf \DIFadd{(KK: 2.5$\sigma$ is the min difference?)}}   
\DIFaddend When a similar
...
with $\epsilon_4$ for
the same number of events, giving values \DIFdelbegin \DIFdel{compatible }\DIFdelend \DIFaddbegin \DIFadd{comparable }\DIFaddend with those
...
leading to a \DIFdelbegin \DIFdel{discrimination of }\DIFdelend \DIFaddbegin \DIFadd{separation between }\DIFaddend the $\mathrm{CP}$-mixed cases
\DIFdelbegin \DIFdel{by up to }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend $\sim 16\sigma$. \DIFaddbegin {\bf \DIFadd{(KK: wording change OK?)}}\DIFaddend \par
...
to test the \DIFdelbegin \DIFdel{sign of $\kpt/\kp$ }\DIFdelend \DIFaddbegin \DIFadd{relative sign of $\kp$ and $\kpt$ }\DIFaddend within that context.
\DIFdelbegin %DIFDELCMD < {\bf %%%
\DIFdel{(KK: maybe reword this sentence a bit\ldots?)}%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend 
