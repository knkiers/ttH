...
%DIF DEL ../manuscript_older/Paper_ttbarH_Draft_3_1_16.tex   Wed Mar  2 13:20:13 2016
%DIF ADD Paper_ttbarH_Draft.tex                              Wed Mar  2 21:08:36 2016
\pdfoutput=1
\usepackage{graphicx}
%\usepackage{dcolumn}
\usepackage{amsfonts}
...
the process $\ppprocess$\DIFaddbegin \DIFadd{, in which the top and antitop both decay
semileptonically}\DIFaddend .
...
\DIFdel{(KK: seems repetitive to say ``gluon fusion'' right away
  again\ldots maybe can change this; maybe OK to leave it?)}%DIFDELCMD < }
%DIFDELCMD < %%%
...
\begin{figure}[H]
%\centering
\DIFdelbeginFL \DIFdelFL{\hspace*{-0.52cm}
}%DIFDELCMD < \subfloat{\includegraphics[scale=0.45]{TP1_10_nuevo.pdf}}
%DIFDELCMD < %%%
\DIFdelFL{\hspace*{-0.006\textwidth}
}%DIFDELCMD < \subfloat{\includegraphics[scale=0.45]{TP1_01_nuevo.pdf}} %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\hspace*{-0.55cm}
%DIF > \subfloat{\includegraphics[scale=0.443]{TP1_10_nuevo.pdf}}
}\subfloat{\includegraphics[scale=0.45]{NEW_TP1_1_0.pdf}}
\DIFaddFL{\hspace*{-0.006\textwidth}
}\subfloat{\includegraphics[scale=0.45]{NEW_TP1_0_1.pdf}} \DIFaddendFL \\
\DIFdelbeginFL \DIFdelFL{\hspace*{-0.45cm}
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\hspace*{-0.55cm}
}\DIFaddendFL %\hspace*{0.03\textwidth}
\DIFdelbeginFL %DIFDELCMD < \subfloat{\includegraphics[scale=0.45]{TP1_11_nuevo.pdf}}
%DIFDELCMD < %%%
\DIFdelFL{\hspace*{-0.006\textwidth}
}%DIFDELCMD < \subfloat{\includegraphics[scale=0.45]{TP1_1-1_nuevo.pdf}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \subfloat{\includegraphics[scale=0.45]{NEW_TP1_1_1.pdf}}
\DIFaddFL{\hspace*{-0.006\textwidth}
...
%\centering
\hspace*{-0.52cm}
\DIFdelbeginFL %DIFDELCMD < \subfloat{\includegraphics[scale=0.45]{TP2_10_nuevo.pdf}}
%DIFDELCMD < %%%
\DIFdelFL{\hspace*{-0.006\textwidth}
}%DIFDELCMD < \subfloat{\includegraphics[scale=0.45]{TP2_01_nuevo.pdf}} %%%
\DIFdelendFL \DIFaddbeginFL \subfloat{\includegraphics[scale=0.45]{NEW_TP2_1_0.pdf}}
\DIFaddFL{\hspace*{-0.006\textwidth}
}\subfloat{\includegraphics[scale=0.45]{NEW_TP2_0_1.pdf}} \DIFaddendFL \\
\DIFdelbeginFL \DIFdelFL{\hspace*{-0.45cm}
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\hspace*{-0.56cm}
}\DIFaddendFL %\hspace*{0.03\textwidth}
\DIFdelbeginFL %DIFDELCMD < \subfloat{\includegraphics[scale=0.45]{TP2_11_nuevo.pdf}}
%DIFDELCMD < %%%
\DIFdelFL{\hspace*{-0.006\textwidth}
}%DIFDELCMD < \subfloat{\includegraphics[scale=0.45]{TP2_1-1_nuevo.pdf}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \subfloat{\includegraphics[scale=0.45]{NEW_TP2_1_1.pdf}}
\DIFaddFL{\hspace*{0.002\textwidth}
}\subfloat{\includegraphics[scale=0.45]{NEW_TP2_1_-1.pdf}}
%DIF > \subfloat{\includegraphics[scale=0.45]{TP2_1-1_nuevo.pdf}}
...
\DIFaddend In order to quantify the \DIFdelbegin \DIFdel{mentioned shifts }\DIFdelend \DIFaddbegin \DIFadd{shifts discussed above}\DIFaddend , we have fitted
the simulated distributions with the following function\DIFdelbegin \DIFdel{proposed in
}\DIFdelend \DIFaddbegin \DIFadd{, which was proposed in
Ref.~}\DIFaddend \cite{Ellis},
%
\beq
\label{eq27}
...
\eeq
%
To the extent that \DIFdelbegin \DIFdel{this equation is exact}\DIFdelend \DIFaddbegin \DIFadd{the above expression is exact, }\DIFaddend we note that
Eq.~(\DIFdelbegin \DIFdel{\ref{eq29}) }\DIFdelend \DIFaddbegin \DIFadd{\ref{eq26}) }{\bf \DIFadd{(KK: corrected the equation number; please check\ldots )}}
\DIFaddend gives $\mathcal{A}(\epsilon_i)=4a_1 \sin\delta$. With
this fitting function, we obtain phase shifts \DIFaddbegin \DIFadd{$\delta$ that
are }\DIFaddend approximately between
$0.9$ and $1$ ($-1$ and $-0.9$) for $\kp=\kpt=1$ ($\kp=-\kpt=1$)\DIFaddbegin \DIFadd{, }\DIFaddend both
for $\epsilon_1$ and $\epsilon_2$.\DIFaddbegin \footnote{\DIFadd{The results for the
  ...
  here. }{\bf \DIFadd{(KK: OK to move this to a footnote?)}}}  \DIFaddend However, the quality of the fits is
not \DIFdelbegin \DIFdel{good enough, being worse in the case of the }\DIFdelend \DIFaddbegin \DIFadd{very good, particularly for }\DIFaddend $\epsilon_1$\DIFdelbegin \DIFdel{distributions for which }\DIFdelend \DIFaddbegin \DIFadd{.  The
}\DIFaddend $\chi^2/\mathrm{d.o.f}$ \DIFdelbegin \DIFdel{is in }\DIFdelend \DIFaddbegin \DIFadd{for the fits in this case are in }\DIFaddend the range
$1.69$-$3.86$, \DIFdelbegin \DIFdel{in contrast to }\DIFdelend \DIFaddbegin \DIFadd{while for $\epsilon_2$ they are in }\DIFaddend the range $0.53$-$1.16$\DIFdelbegin \DIFdel{obtained for $\epsilon_2$.  The main impact on the }\DIFdelend \DIFaddbegin \DIFadd{.
}{\bf \DIFadd{(KK: Did we do 4 fits for each $\epsilon$?  Or only 2?  If only 2, maybe
  ...
  mixed-CP scenarios.)}}
\DIFadd{The }\DIFaddend deviation from the functional
form proposed in Eq.~(\ref{eq27}) appears to be \DIFdelbegin \DIFdel{caused by }\DIFdelend \DIFaddbegin \DIFadd{due primarily to }\DIFaddend the $\Delta
R_{ll}$ cut \DIFaddbegin \DIFadd{that }\DIFaddend we have imposed. In fact, when this cut is turned off\DIFaddbegin \DIFadd{, }\DIFaddend the
above ranges for \DIFaddbegin \DIFadd{the }\DIFaddend $\chi^2/\mathrm{d.o.f}$ \DIFdelbegin \DIFdel{modify to }\DIFdelend \DIFaddbegin \DIFadd{become }\DIFaddend $0.75$-$1.14$ and
$0.44$-$1.07$ for the $\epsilon_1$ and $\epsilon_2$ distributions\DIFdelbegin \DIFdel{respectively. In }\DIFdelend \DIFaddbegin \DIFadd{,
respectively. }\DIFaddend Tables \ref{table3} and \ref{table4} \DIFdelbegin \DIFdel{we }\DIFdelend list the
results of the fits obtained when the $\Delta R_{\ell\ell}$ cut is
relaxed.  \DIFdelbegin \DIFdel{The results for the TP $\epsilon_3$ are pretty similar to
those for $\epsilon_2$ but with opposite sign in the phase shifts of the $\mathrm{CP}$-mixed hypotheses and hence we do not include them
here. In addition, we show in Fig.~\ref{fig7} the angular
distributions along with the fit curves for both $\mathrm{CP}$-mixed
cases when no cut is
imposed in $\Delta R_{\ell\ell}$.
}\DIFdelend \DIFaddbegin \DIFadd{Figure~\ref{fig7} shows the corresponding
...
\caption{Fit results for the angular distribution $d\sigma/(\sigma
  d\Delta\phi_1(n_t,n_{\tbar}))$ \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL related to the TP
  $\epsilon_1=\epsilon(t,\tbar,n_t,n_{\tbar})$\DIFdelbeginFL \DIFdelFL{when }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{) with }\DIFaddendFL the $\Delta
  R_{\ell\ell}$ \DIFdelbeginFL \DIFdelFL{is }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cut }\DIFaddendFL turned off. Note that the \DIFaddbeginFL \DIFaddFL{sign of the }\DIFaddendFL parameter $a_1$
  \DIFdelbeginFL \DIFdelFL{change
  its sign }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{changes }\DIFaddendFL for $\kp=0,\kp=1$\DIFaddbeginFL \DIFaddFL{, compared to the
  other cases}\DIFaddendFL .  \DIFaddbeginFL {\bf \DIFaddFL{(KK: Do we restrict $\delta$ to be between
    $\pm \pi/2$?  If not, we could require $a_1$ to always
    be positive, and absorb the sign change by a shift of $\delta$ by $\pi$
    or ssomething.  Maybe we should clarify the range allowed for $\delta$.)}} \DIFaddendFL }
\label{table3}
\begin{center}
...
\begin{table}[H]
\caption{Fit results for the angular distribution $d\sigma/(\sigma
  d\Delta\phi_2(n_t,n_{\tbar}))$ \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL related to the TP
  $\epsilon_2=\epsilon(Q,\tbar,n_t,n_{\tbar})$\DIFdelbeginFL \DIFdelFL{when }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{), with }\DIFaddendFL the $\Delta
  R_{\ell\ell}$ \DIFdelbeginFL \DIFdelFL{is }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cut }\DIFaddendFL turned off. \DIFdelbeginFL \DIFdelFL{Note that }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{As was }\DIFaddendFL the \DIFaddbeginFL \DIFaddFL{case in Table~\ref{table3},
  the sign of the }\DIFaddendFL parameter $a_1$ \DIFdelbeginFL \DIFdelFL{change
  its sign }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{changes }\DIFaddendFL for
  $\kp=0,\kp=1$.}
\label{table4}
\begin{center}
\begin{tabular}{|C{1cm}|C{1cm}||C{3cm}|C{3cm}|C{3cm}|}
...
\end{table}
\par
\DIFdelbegin \DIFdel{From Tables \ref{table3} and \ref{table4} we see that the
parameter $\delta$ is sensitive not only to the modulus of $\kpt$ but
...
\caption{Angular distributions $d\sigma/(\sigma
  ...
  $\epsilon_1=\TPa$ and $\epsilon_2=\TPb$\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL respectively\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL for the
  $\mathrm{CP}$-mixed cases $\kp=\kpt=1$ (left) and $\kp=-\kpt=1$
  (right)\DIFdelbeginFL \DIFdelFL{when the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{.  The }\DIFaddendFL $\Delta R_{\ell\ell}$ cut \DIFdelbeginFL \DIFdelFL{is }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{was }\DIFaddendFL turned off \DIFaddbeginFL \DIFaddFL{when
  generating these results}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{Also, the
  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The
  }\DIFaddendFL corresponding fit curves \DIFaddbeginFL [\DIFaddFL{see Eq.~(\ref{eq27})}] \DIFaddendFL are displayed in red.}
\label{fig7}
\end{figure}
\end{center}
\par
\DIFdelbegin \DIFdel{Concerning the angular distributions that can be related to the
combination $\epsilon_4$}\DIFdelend \DIFaddbegin 

\DIFadd{In Sec.~\ref{sec3.1} we defined a fourth triple product,
$\epsilon_4 = \epsilon_3-\epsilon_2$.  We have constructed
an angular distribution related to this TP as well.
Specifically}\DIFaddend , we have \DIFdelbegin \DIFdel{analized }\DIFdelend \DIFaddbegin \DIFadd{analyzed }\DIFaddend the
$\Delta\phi(n_t,n_{\tbar})$ distribution in the $Q$ rest frame\DIFdelbegin \DIFdel{with
}\DIFdelend \DIFaddbegin \DIFadd{,
taking
}\DIFaddend $H$ \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{to define }\DIFaddend the $z$-axis\DIFaddbegin \DIFadd{.  By studying the distributions
}\DIFaddend for various values of $\kp$ and $\kpt$\DIFdelbegin \DIFdel{. We }\DIFdelend \DIFaddbegin \DIFadd{, we }\DIFaddend have
found that \DIFdelbegin \DIFdel{these distributions are not }\DIFdelend \DIFaddbegin \DIFadd{they are not well }\DIFaddend described by Eq.~(\ref{eq27})
and their range of variation is larger than that of the distributions
displayed in Figs.~\ref{fig5} and \ref{fig6}.
\DIFaddbegin {\bf \DIFadd{(KK: I'm not sure what this means.  Are they less cosine-like?
  Or are the amplitudes or phase shifts larger than those for the other
  TPs?)}}
\DIFaddend Unlike the distributions
related to $\epsilon_1$-$\epsilon_3$, those arising from $\epsilon_4$
exhibit small changes in their shapes \DIFaddbegin {\bf \DIFadd{(KK: I don't think I understand
  this, since we said that the range of variation was larger in the last
  sentence.  I think I'm missing something :) )}} \DIFaddend for all the considered
...
distributions leads to higher values for the asymmetry (as can be \DIFdelbegin \DIFdel{sees
}\DIFdelend \DIFaddbegin \DIFadd{seen
}\DIFaddend from Tables \ref{table1} and \ref{table2}) even when the changes in
the respective shapes are smaller \DIFaddbegin {\bf \DIFadd{(KK: I don't understand what
  this means.)}} \DIFaddend than in the case of the
distributions described by Eq.~(\ref{eq27}).  \par
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Mean value}
\label{sec3.3}
We turn now to consider the last type of \DIFdelbegin \DIFdel{observables constructed }\DIFdelend \DIFaddbegin \DIFadd{observable that we will
construct }\DIFaddend from the TPs\DIFdelbegin \DIFdel{that are sensitive to $\kpt$}\DIFdelend , the mean value. \DIFaddbegin \DIFadd{As was the case for the
observables considered in Secs.~\ref{sec3.1} and \ref{sec3.2},
the mean value is sensitive to $\kpt$.  }\DIFaddend Given a certain
...
\eeq
%
\DIFaddbegin {\bf \DIFadd{(KK: PRD might change these to }[\DIFadd{\ldots}]\DIFadd{.)}}
\DIFaddend where $\Phi$ is the \DIFdelbegin \DIFdel{Lorentz invariant }\DIFdelend \DIFaddbegin \DIFadd{Lorentz-invariant }\DIFaddend phase space corresponding to the
final state
$b\,\ell^+\nu_{\ell}\,\bbar\,\ell^-\bar{\nu}_{\ell}H$. From
Eq.~(\ref{eq20}) we see that only the terms linear in \DIFaddbegin \DIFadd{(both) }\DIFaddend $\kp$ and $\kpt$
will contribute to the mean value\DIFdelbegin \DIFdel{, so that }\DIFdelend \DIFaddbegin \DIFadd{.  Thus, }\DIFaddend we expect this observable
to be sensitive not only to the \DIFdelbegin \DIFdel{value }\DIFdelend \DIFaddbegin \DIFadd{magnitude of $\kp\kpt$,
}\DIFaddend but also to the relative sign of
the couplings.\par
\DIFaddbegin 

\DIFaddend The results obtained for the TPs $\epsilon_1=
...
displayed in Table\DIFdelbegin \DIFdel{\ref{table5}, where }\DIFdelend \DIFaddbegin \DIFadd{~\ref{table5}.  For each TP }\DIFaddend we list the 
\DIFdelbegin \DIFdel{deviation of the
mean values from the
SM case ($\kpt = 0$) in terms of the
}\DIFdelend \DIFaddbegin \DIFadd{mean value divided by the
}\DIFaddend corresponding statistical uncertainty\DIFdelbegin \DIFdel{of the estimator of $\langle
\epsilon \rangle$, $\bar{\epsilon}$}\DIFdelend .
\DIFdelbegin %DIFDELCMD < \renewcommand{\arraystretch}{1.6}
%DIFDELCMD < \begin{table}[H]
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Mean values obtained for the TPs $\epsilon_{1,2,3}$ for the
  ...
  simulated events.}}
%DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{table5}%%%
%DIFDELCMD < \begin{center}
%DIFDELCMD < \begin{tabular}{|C{1cm}|C{1cm}||C{2cm}||C{2cm}||C{2cm}|}
%DIFDELCMD < %%%
%DIF < \hhline{|=====|}
%DIFDELCMD < \hhline{|-----|}
%DIFDELCMD < %%%
...
\DIFdelFL{$\langle \epsilon_1 \rangle /\sigma_{\bar{\epsilon}_1}$ }%DIFDELCMD < & %%%
\DIFdelFL{$\langle \epsilon_2 \rangle /\sigma_{\bar{\epsilon}_2}$ }%DIFDELCMD < & %%%
\DIFdelFL{$\langle \epsilon_3 \rangle /\sigma_{\bar{\epsilon}_3}$ }%DIFDELCMD < \\ 
%DIFDELCMD < \hhline{|=====|} 
%DIFDELCMD < %%%
%DIF < \hhline{|-----|}
%DIF < \vspace*{.5mm}
%DIFDELCMD < \renewcommand{\arraystretch}{1.0}
%DIFDELCMD < %%%
\DIFdelFL{$1$~ }%DIFDELCMD < & %%%
\DIFdelFL{$-1$~~~ }%DIFDELCMD < & %%%
\DIFdelFL{$4.26$~ }%DIFDELCMD < & %%%
\DIFdelFL{$4.94$~ }%DIFDELCMD < & %%%
\DIFdelFL{$-5.81$~~~}%DIFDELCMD < \\[0.6mm]
%DIFDELCMD < \hline
%DIFDELCMD < %%%
\DIFdelFL{$1$ }%DIFDELCMD < & %%%
...
\DIFdelFL{$1.25$~}%DIFDELCMD < \\[0.6mm]
%DIFDELCMD < \hline
%DIFDELCMD < %%%
\DIFdelFL{$1$ }%DIFDELCMD < & %%%
\DIFdelFL{$1$~ }%DIFDELCMD < & %%%
\DIFdelFL{$-7.98$~~~ }%DIFDELCMD < & %%%
\DIFdelFL{$-8.83$~~~}%DIFDELCMD < & %%%
\DIFdelFL{$8.75$~}%DIFDELCMD < \\[0.6mm]
%DIFDELCMD < \hhline{|=====|}
%DIFDELCMD < %%%
%DIF < \hhline{|-----|}
...
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin {\bf \DIFadd{(KK: I simplified this sentence and
...
%DIF > A deviation  of the estimator of $\langle
%DIF > \epsilon \rangle$, $\bar{\epsilon}$.
\DIFaddend We see that the three observables are capable of \DIFdelbegin \DIFdel{disinguishing }\DIFdelend \DIFaddbegin \DIFadd{distinguishing }\DIFaddend the SM
case from both $\mathrm{CP}$-mixed cases. \DIFdelbegin \DIFdel{Further,
}\DIFdelend \DIFaddbegin \DIFadd{Furthermore,
two $\mathrm{CP}$-mixed cases
are clearly disentangled, since the }\DIFaddend the observables are
sensitive to the sign of $\kpt$\DIFdelbegin \DIFdel{and the two $\mathrm{CP}$-mixed cases
are also clearly disentangled}\DIFdelend . The observables $\langle \epsilon_2
...
Table \ref{table5}, \DIFdelbegin \DIFdel{with $-4.32, 1.11$ }\DIFdelend \DIFaddbegin \DIFadd{yielding $-4.32$, $1.11$ }\DIFaddend and $7.23$ for the cases
$(\kp=1,\kpt=-1,0,1)$\DIFaddbegin \DIFadd{, }\DIFaddend respectively. As with the asymmetry, the
\DIFdelbegin \DIFdel{hypotheses }\DIFdelend \DIFaddbegin \DIFadd{purely }\DIFaddend $\mathrm{CP}$-even and \DIFaddbegin \DIFadd{purely }\DIFaddend $\mathrm{CP}$-odd \DIFaddbegin \DIFadd{cases }\DIFaddend cannot be
distinguished by the mean value\DIFdelbegin \DIFdel{that is also }\DIFdelend \DIFaddbegin \DIFadd{, since it is }\DIFaddend linear in both $\kp$ and
$\kpt$ (see Eqs.~(\ref{eq20}) and (\ref{eq28})). \DIFdelbegin \DIFdel{By taking into
account the results
given in }\DIFdelend \DIFaddbegin \DIFadd{Comparing the results
in Table~\ref{table5} with
the results presented in }\DIFaddend Sec.~\ref{sec3.1}, we can conclude that
the sensitivity to the NP contribution is smaller for the mean values
\DIFdelbegin \DIFdel{than for the asymmetries }\DIFdelend of the TPs under consideration
\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{than for the corresponding asymmetries.
}


\renewcommand{\arraystretch}{1.6}
\begin{table}[H]
\caption{\DIFaddFL{Mean values obtained for the TPs $\epsilon_{1,2,3}$ for the
...
%DIF > \hhline{|=====|}
\hhline{|-----|}
\DIFaddFL{$\kappa_t$ }& \DIFaddFL{$\tilde{\kappa}_t$ }& \DIFaddFL{$\langle \epsilon_1 \rangle /\sigma_{\bar{\epsilon}_1}$ }& \DIFaddFL{$\langle \epsilon_2 \rangle /\sigma_{\bar{\epsilon}_2}$ }& \DIFaddFL{$\langle \epsilon_3 \rangle /\sigma_{\bar{\epsilon}_3}$ }\\ 
\hhline{|=====|} 
%DIF > \hhline{|-----|}
%DIF > \vspace*{.5mm}
\renewcommand{\arraystretch}{1.0}
\DIFaddFL{$1$~ }& \DIFaddFL{$-1$~~~ }& \DIFaddFL{$4.26$~ }& \DIFaddFL{$4.94$~ }& \DIFaddFL{$-5.81$~~~}\\[0.6mm]
\hline
\DIFaddFL{$1$ }& \DIFaddFL{$0$~ }& \DIFaddFL{$-0.91$~~~ }& \DIFaddFL{$-0.22$~~~ }& \DIFaddFL{$1.25$~}\\[0.6mm]
\hline
\DIFaddFL{$1$ }& \DIFaddFL{$1$~ }& \DIFaddFL{$-7.98$~~~ }& \DIFaddFL{$-8.83$~~~}& \DIFaddFL{$8.75$~}\\[0.6mm]
\hhline{|=====|}
%DIF > \hhline{|-----|}
\end{tabular}
\end{center} 
...
and $Q$ and the spin vectors $n_t$ and $n_{\tbar}$ \DIFdelbegin \DIFdel{given }\DIFdelend \DIFaddbegin [\DIFadd{defined }\DIFaddend in
Eqs.~(\ref{eq3})-(\ref{eq4}\DIFdelbegin \DIFdel{).  In fact}\DIFdelend \DIFaddbegin ]\DIFadd{.  Furthermore}\DIFaddend , we have described the general
form of the differential cross-section in terms of these vectors in
Eq.~(\ref{eq20}). \DIFdelbegin \DIFdel{Here, we will take into account another
}\DIFdelend \DIFaddbegin \DIFadd{In this section we consider other
}\DIFaddend possibilities for the choice of the vectors from which the
...
\beq
\label{eq30}
\TPb = \frac{m^2_t}{(t\cdot \ell^+)(\tbar\cdot \ell^-)}\left(\epsilon(t,\tbar,\ell^-\!,\ell^+)+\epsilon(H,\tbar,\ell^-\!,\ell^+)+\frac{(t\cdot\ell^+)}{m^2_t}\epsilon(H,\tbar,t,\ell^-\!)\right) \DIFaddbegin \DIFadd{\,,
}\DIFaddend \eeq
%
\vspace*{1mm}
\beq
...
Secs.~\ref{sec3.1}-\ref{sec3.3}.  \DIFdelbegin \DIFdel{In the first place}\DIFdelend \DIFaddbegin \DIFadd{Now}\DIFaddend , we have found
that TPs not including both \DIFaddbegin \DIFadd{the }\DIFaddend lepton and anti-lepton momenta \DIFdelbegin \DIFdel{present a
negligible sensitivity , and then we will }\DIFdelend \DIFaddbegin \DIFadd{yield
negligible sensitivity to the value of $\kpt$.  For this reason,
we }\DIFaddend concentrate here on the
results obtained for the remaining TPs,
\DIFdelbegin \DIFdel{$\epsilon_5\equiv\epsilon(t,\tbar,\ell^-\!,\ell^+),
\epsilon_6\equiv\epsilon(H,t,\ell^-\!,\ell^+)$ and $\epsilon_7\equiv
\epsilon(H,\tbar,\ell^-\!,\ell^+)$.
The Tables \ref{table6} and
\ref{table7} summarize the results for these TPs.}\DIFdelend %DIF > 
\DIFaddbegin \beq
\label{eqa5}
\DIFadd{\epsilon_5\equiv\epsilon(t,}\tbar\DIFadd{,\ell^-\!,\ell^+) \,,
}\vspace*{1mm}
\eeq
%DIF > 
\beq
\label{eqa6}
\DIFadd{\epsilon_6\equiv\epsilon(H,t,\ell^-\!,\ell^+) \,,
}\vspace*{1mm}
\eeq
%DIF > 
\beq
\label{eqa7}
\DIFadd{\epsilon_7\equiv\epsilon(H,}\tbar\DIFadd{,\ell^-\!,\ell^+) \,.
}\vspace*{1mm}
\eeq
%DIF > 
    {\bf \DIFadd{(KK: Should we clarify somehow that $\epsilon_{4,5,6,7}$ are not the same as
      $\epsilon_{4,5,6,7}$ in Eq.~(20)?  That seems like a potential source of confusion.)}}

\DIFaddend \renewcommand{\arraystretch}{1.6}
\begin{table}[H]
...
\DIFadd{Tables~\ref{table6} and
\ref{table7} summarize the results for the TPs $\epsilon_{5,6,7}$.
    }{\bf \DIFadd{(KK: Should we interchange the two tables?  In the previous section
      we considered asymmetries first, then mean values.  Seems like it might
      flow better to follow that same order here.)}}
\DIFaddend We see that the TP $\epsilon_5$ gives rise to asymmetries and mean
...
for which the asymmetries and mean values are comparable \DIFaddbegin \DIFadd{for the TPs
}\DIFaddend (see Tables
\ref{table1} and \ref{table5}). We also note that the asymmetry for
$\epsilon_5$ is exactly the same as for $\epsilon_1$\DIFaddbegin \DIFadd{, }\DIFaddend as was expected
...
Tables\DIFaddbegin \DIFadd{~}\DIFaddend \ref{table5} and \ref{table6} that the TPs $\epsilon_{1,2,3}$
