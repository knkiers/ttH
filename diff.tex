...
%DIF DEL ../manuscript_older/Paper_ttbarH_Draft_3_4_16.tex   Fri Mar  4 16:00:28 2016
%DIF ADD Paper_ttbarH_Draft.tex                              Sat Mar  5 00:46:21 2016
\pdfoutput=1
\usepackage{graphicx}
%\usepackage{dcolumn}
\usepackage{amsfonts}
...
by more than the $\sim 20\sigma$ statistical level ({\bf N: OK to keep ``disentangle" here or do we need to change to something like ``can disentangle the hypotheses with effective separations of order...?"}).  In
...
asymmetry to distinguish ({\bf N: Do we add ``effectively" ?}) between the
...
for $\epsilon_1$ and $\epsilon_2$.\footnote{The results for the
  ...
  here.}  However, the quality of the fits in the four considered scenarios is
...
the angle $\Delta\phi_{\ell^-\ell^+}$ [see the discussion following
  Eq.~(\ref{eq25})],
...
will be sensitive to the sign of the pseudoscalar coupling. This
angular variable is the same as that proposed in Ref.~\cite{Ellis} as a
...
form given in Eq.~(\ref{eq27}). The \DIFdelbegin \DIFdel{ascociated }\DIFdelend \DIFaddbegin \DIFadd{associated }\DIFaddend shifts ($\delta$)
...
distribution since this distribution
is constrained by the asymmetry $\mathcal{A}(\epsilon_5)$
[via Eq.~(\ref{eq26})],
which in turn is equal to
$\mathcal{A}(\epsilon_1)$. Also, we note that $\mathcal{A}(\epsilon_5)$ is slightly smaller 
than $\mathcal{A}(\epsilon_2)$,
as can be seen from Table\DIFaddbegin \DIFadd{~}\DIFaddend \ref{table1}.
\par
...
Having defined the TPs $\epsilon_5$-$\epsilon_7$, one can
...
of the momenta of the leptons instead of using the spin vectors. The
...
distributions, their range of variation {\bf (KK: I'm not sure what
  this means)} is larger than in the case of
the distributions related to $\epsilon_5$-$\epsilon_7$ but exhibit
smaller changes in their shapes for the different scenarios and for
this reason we do not include here the corresponding plots.
{\bf (KK: might need to reword this a bit after clarifying what 
...
in Eqs.~(\ref{eq3})-(\ref{eq4}), we have
%
\beq
\label{eq34}
...
$t\tbar H (H\rightarrow b\bbar)$ at the LHC~\cite{dosSantos}.
 In all these cases the corresponding
...
$5.6\sigma$ ({\bf N: Do we add ``effectively" distinguish?}).\par
...
\DIFaddbegin \DIFadd{in $\ppprocess$ }\DIFaddend that are useful for
disentangling the relative sign between the scalar ($\kp$) and a
potential pseudoscalar ($\kpt$) top-Higgs couplings\DIFdelbegin \DIFdel{in the $t\tbar H$
production with both tops decaying leptonically}\DIFdelend .
We have tested the
sensitivity of the \DIFdelbegin \DIFdel{proposed observables}\DIFdelend \DIFaddbegin \DIFadd{various triple product correlations
}{\bf \DIFadd{(KK: OK? ``observables'' was repeated)}}
\DIFaddend by considering three types of
observables: asymmetries, \DIFdelbegin \DIFdel{mean values }\DIFdelend \DIFaddbegin \DIFadd{angular distributions, and mean values.
Using these observables, we have examined several benchmark
scenarios, focusing in particular on the SM ($\kp=1$ and $\kpt=0$)
and on two ``$\mathrm{CP}$-mixed'' scenarios ($\kp=1$ }\DIFaddend and \DIFdelbegin \DIFdel{angular distributions}\DIFdelend \DIFaddbegin \DIFadd{$\kpt=\pm1$)}\DIFaddend .
\par
...
connected by the spin vectors of the top and antitop\DIFaddbegin \DIFadd{, }\DIFaddend which are given
in terms of the momenta of the leptons in the final state. Moreover,
we have \DIFdelbegin \DIFdel{indentified }\DIFdelend \DIFaddbegin \DIFadd{identified }\DIFaddend the terms linear in $\kp$ and $\kpt$ as those
...
We have found
that $\epsilon_{1,2,3}$ allow \DIFaddbegin \DIFadd{one }\DIFaddend to distinguish between the
$\mathrm{CP}$-mixed \DIFdelbegin \DIFdel{hypotheses }\DIFdelend \DIFaddbegin \DIFadd{scenarios }\DIFaddend by more than $\sim 20\sigma$ in the
...
also sensitive to the value of $\kp$ and $\kpt$\DIFaddbegin \DIFadd{, }\DIFaddend exhibiting a phase
shift that varies according to the values taken by these couplings.
\DIFdelbegin \DIFdel{On
the other hand, by }\DIFdelend \DIFaddbegin \DIFadd{By }\DIFaddend exploring TPs that incorporate the momenta of the
Higgs and the leptons instead of the spin vectors\DIFdelbegin \DIFdel{we can conclude }\DIFdelend \DIFaddbegin \DIFadd{, we have concluded }\DIFaddend that
the observables studied here appear to be more sensitive when the spin
vectors are used.\par

\DIFdelbegin \DIFdel{On the other hand, we have }\DIFdelend \DIFaddbegin \DIFadd{We have also }\DIFaddend proposed a combination of the TPs\DIFdelbegin \DIFdel{considered in the first place}\DIFdelend ,
$\epsilon_4\equiv \epsilon_3-\epsilon_2$,
\DIFdelbegin \DIFdel{that exhibits the highest sensitivity with the }\DIFdelend \DIFaddbegin \DIFadd{which has a greater sensitivity than $\epsilon_1$-$\epsilon_3$.
With $1\times 10^5$ events, for example, the asymmetry associated
with this TP gives an effective }\DIFaddend separation 
between the $\mathrm{CP}$-mixed \DIFdelbegin \DIFdel{hypotheses being increased
}\DIFdelend \DIFaddbegin \DIFadd{scenarios that
exceeds those coming from $\epsilon_1$-$\epsilon_3$
}\DIFaddend by at least $3\sigma$\DIFdelbegin \DIFdel{in the asymmetry for $1 \times 10^5$ events with
respect to $\mathcal{A}(\epsilon_{1,2,3})$.Again, when }\DIFdelend \DIFaddbegin \DIFadd{.  }{\bf \DIFadd{(KK: please check my rewording for accuracy.)}}
\DIFadd{When }\DIFaddend a similar
combination is constructed by using the leptons\DIFaddbegin \DIFadd{' }\DIFaddend momenta instead of the
spin vectors ($\epsilon_8$), the sensitivity in the asymmetry is
decreased by $\sim 3\sigma$ compared to \DIFdelbegin \DIFdel{$\mathcal{A}(\epsilon_4)$ }\DIFdelend \DIFaddbegin \DIFadd{the asymmetry associated
with $\epsilon_4$ }\DIFaddend for
the same number of events, giving values compatible with those
obtained for the \DIFdelbegin \DIFdel{asymmetry }\DIFdelend \DIFaddbegin \DIFadd{asymmetries }\DIFaddend of $\epsilon_2$ and $\epsilon_3$. \par
...
\DIFaddbegin \DIFadd{momenta }\DIFaddend by the $b$ and $\bbar$ momenta ($\epsilon_9$), whereas the second
includes the visible part of the $t$ and $\tbar$ momenta
($\epsilon_{10}$). We have \DIFdelbegin \DIFdel{encountered }\DIFdelend \DIFaddbegin \DIFadd{found }\DIFaddend that the latter is the \DIFdelbegin \DIFdel{most
sensitive }\DIFdelend \DIFaddbegin \DIFadd{more
sensitive of the two,
}\DIFaddend leading to a discrimination of the $\mathrm{CP}$-mixed cases
\DIFdelbegin \DIFdel{under analysis }\DIFdelend by up to $\sim 16\sigma$.\par

Finally\DIFaddbegin \DIFadd{, }\DIFaddend we have discussed the \DIFaddbegin \DIFadd{experimental }{\bf \DIFadd{(KK: OK?)}}
\DIFaddend feasibility of the most sensitive
observables proposed here. We have found that with $5\times 10^3$ and
$1\times 10^4$ events\DIFdelbegin \DIFdel{respectively the observables
$\mathcal{A}(\epsilon_4)$ and $\mathcal{A}(\epsilon_{10})$ }\DIFdelend \DIFaddbegin \DIFadd{, respectively, the asymmetries
associated with $\epsilon_4$ and $\epsilon_{10}$ }\DIFaddend are still
useful for testing the hypotheses $(\kp=1,\kpt=\pm 1)$\DIFaddbegin \DIFadd{, }\DIFaddend giving rise to
separations of order $\sim 6\sigma$. These \DIFdelbegin \DIFdel{required number of events
would be reachable in principle in }\DIFdelend \DIFaddbegin \DIFadd{numbers of events
are within reach in }\DIFaddend the short term \DIFdelbegin \DIFdel{by the LHCand hence
the capability of the observables studied here for testing }\DIFdelend \DIFaddbegin \DIFadd{at the LHC,
so that these observables could in principle be used
to test }\DIFaddend the sign of $\kpt/\kp$ \DIFdelbegin \DIFdel{could be probed in }\DIFdelend \DIFaddbegin \DIFadd{within }\DIFaddend that context.
\DIFaddbegin {\bf \DIFadd{(KK: maybe reword this sentence a bit\ldots?)}}
\DIFaddend 
...
\DIFaddbegin \DIFadd{DC, EG and }\DIFaddend KK was supported by the U.S.  National Science Foundation under Grant
PHY-1215785\DIFdelbegin \DIFdel{. KK also acknowledges sabbatical support from Taylor
University}\DIFdelend .
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{\refname}
