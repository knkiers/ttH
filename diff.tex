...
%DIF DEL ../manuscript_older/Paper_ttbarH_Draft_2_25_16.tex   Thu Feb 25 11:59:53 2016
%DIF ADD Paper_ttbarH_Draft.tex                               Sat Feb 27 15:49:11 2016
\pdfoutput=1
\usepackage{graphicx}
%\usepackage{dcolumn}
\usepackage{amsfonts}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFdelbegin \DIFdel{Let us focus now in }\DIFdelend \DIFaddbegin 

\subsection{\DIFadd{Triple products}}
{\bf \DIFadd{(KK: maybe we should change this title for the subsection\ldots.)}}

\DIFadd{The expression derived above for the scattering cross
section }[\DIFadd{see Eq.~(\ref{eq2}), as well as Eq.~(\ref{eq17})}]
\DIFadd{provides significant insight into
how one might analyze $\ppprocess$ in order to determine
the nature of the top-Higgs coupling.  In particular,
let us focus on }\DIFaddend the production amplitude\DIFaddbegin \DIFadd{,
$\mathcal{M}(g_ag_b \to t^i(n_t)\tbar^j(n_{\tbar})H))$, which forms
part of the overall amplitude }\DIFaddend in Eq.~(\ref{eq17})\DIFdelbegin \DIFdel{needed for computing the production }\DIFdelend \DIFaddbegin \DIFadd{.
The absolute value squared of the production amplitude
is used to determine $d\sigma(gg\to t(n_t)\tbar (n_{\tbar})H)$,
which in turn forms }\DIFaddend part of the \DIFdelbegin \DIFdel{unpolarized
differential cross-section.By summing }\DIFdelend \DIFaddbegin \DIFadd{expression for the ``factorized'' cross section
in Eq.~(\ref{eq2}).  Summing }\DIFaddend over colour and gluon indices
we have
%
\beq
\label{eq18}
...
\eeq
%
where we have separated the colour structure of each diagram by \DIFdelbegin \DIFdel{using
the definitions }\DIFdelend \DIFaddbegin \DIFadd{defining
}{\bf \DIFadd{(KK: do these need $k$ subscripts?)}}
\DIFaddend $\mathcal{S}^{ab,ij}= C^{ab,ij}_k \mathcal{S}$ and
$\mathcal{P}^{ab,ij}= C^{ab,ij}_k \mathcal{P}$
\DIFaddbegin [\DIFadd{see Eqs.~(\ref{eq5}) and (\ref{eq16})}]\DIFaddend . Also, the factors
$g^2_s m_t/v$ and $-ig^2_s m_t/v$ arising from the vertices of the \DIFdelbegin \DIFdel{t-
and s-channel }\DIFdelend \DIFaddbegin \DIFadd{$t$-
and $s$-channel }\DIFaddend diagrams respectively have been included in the
...
C^{ab,ij*}_r$ is real and \DIFaddbegin \DIFadd{where }\DIFaddend $\tilde{\mathcal{P}}_r = \gamma^0
\mathcal{P}_r \gamma^0$ \DIFaddbegin {\bf \DIFadd{(KK: wouldn't this be
  $\mathcal{P}_r^\dagger$? I didn't look for this in the notes
  yet, so maybe I'm just missing something)}}\DIFaddend .
The only \DIFdelbegin \DIFdel{contributing terms }\DIFdelend \DIFaddbegin \DIFadd{terms that yield non-zero contributions
in the above sum }\DIFaddend are those with an
odd number of $\gamma^5$ matrices\DIFdelbegin \DIFdel{and will lead to triple-products
}\DIFdelend \DIFaddbegin \DIFadd{; these lead to triple-product
}\DIFaddend (TP) structures
\DIFdelbegin \DIFdel{, i.e. contractions between the levi-civita tensor
$\epsilon_{\alpha\beta\gamma\delta}$ with four momenta }\DIFdelend \DIFaddbegin \DIFadd{of the form $\epsilon_{\alpha\beta\gamma\delta}\,p^{\alpha}_ap^{\beta}_bp^{\gamma}_cp^{\delta}_d$,
where $p_a$-$p_d$ represent various four momenta associated with the process}\DIFaddend .
In contrast,
it can be seen from Eq.~(\ref{eq18}) that the terms proportional to
$\kp^2$ and $\tilde{\kappa}^2_t$ \DIFdelbegin \DIFdel{contain }\DIFdelend \DIFaddbegin \DIFadd{descend from traces containing
}\DIFaddend an even number of $\gamma^5$
matrices and can be written in terms of scalar products of the
available momenta.\par
%\vspace*{-2mm}

\DIFdelbegin \DIFdel{In order to state this more clearly, we will consider the form of the
differential cross-section }\DIFdelend \DIFaddbegin \DIFadd{With the above considerations in mind, it is useful to
write a general expression for the differential cross
section $d\sigma(gg\to t(n_t)\tbar (n_{\tbar})H)$
}\DIFaddend in terms of the momenta $q=(q_1-q_2)/2$,
$Q=(q_1+q_2)/2$, $t$, $\bar{t}$, $n_t$ and $n_{\tbar}$, where
$q_{1,2}$ denote the momenta of the \DIFaddbegin \DIFadd{initial-state }\DIFaddend gluons. Note that with this
choice, $q\cdot Q=0$ and $Q^2=-q^2=M^2_{t\tbar H}/4$, where $M_{t\tbar
  H}$ is the invariant mass of the \DIFdelbegin \DIFdel{system }\DIFdelend $t\tbar H$ \DIFdelbegin \DIFdel{. From these six
vectors, fifteen }\DIFdelend \DIFaddbegin \DIFadd{system.
Fifteen }\DIFaddend TPs can be \DIFdelbegin \DIFdel{built}\footnote{\DIFdel{We note that these fifteen
  TPs are not linearly independent (see the epsilon relations
  discussed in \mbox{%DIFAUXCMD
\cite{identities}}%DIFAUXCMD
).}}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{,and the general form of the
production differential cross-section will be given by
}\DIFdelend \DIFaddbegin \DIFadd{constructed from these six
four-vectors,}\footnote{\DIFadd{We note that these fifteen
  TPs are not linearly independent (see the epsilon relations
  discussed in Ref.~\mbox{%DIFAUXCMD
\cite{identities}}%DIFAUXCMD
).}} \DIFadd{so that
}\DIFaddend %
\beq
...
denotes the $l$th TP \DIFdelbegin \DIFdel{, the Roman indices }\DIFdelend \DIFaddbegin \DIFadd{(we adopt the convention $\epsilon_{0123}=+1$) and where $p_i$ and $p_j$ }\DIFaddend refer to any of the
\DIFdelbegin \DIFdel{involved
momentaand we adopt the convention $\epsilon_{0123}=+1$}\DIFdelend \DIFaddbegin \DIFadd{six momenta}\DIFaddend .  The
...
cross-section, whereas the \DIFdelbegin \DIFdel{TPs }\DIFdelend \DIFaddbegin \DIFadd{TP }\DIFaddend terms will be sensitive to the sign of
the anomalous coupling $\kpt$. \DIFdelbegin \DIFdel{From }\DIFdelend \DIFaddbegin \DIFadd{Of }\DIFaddend the fifteen TPs mentioned above,
we will focus on those that \DIFdelbegin \DIFdel{do not include $q$ and contain both }\DIFdelend \DIFaddbegin \DIFadd{contain both of the }\DIFaddend spin
vectors $n_t$ and $n_{\tbar}$\DIFdelbegin \DIFdel{. The former is motivated in }\DIFdelend \DIFaddbegin \DIFadd{, but do not include $q$.
The decision not to consider $q$-dependent TPs is motivated by }\DIFaddend the fact
that $q$ cannot be \DIFdelbegin \DIFdel{eliminated }\DIFdelend \DIFaddbegin \DIFadd{expressed }\DIFaddend in terms of the momenta of final state
particles (\DIFdelbegin \DIFdel{like }\DIFdelend \DIFaddbegin \DIFadd{as }\DIFaddend $Q$ \DIFdelbegin \DIFdel{in virtue of momentum-energy }\DIFdelend \DIFaddbegin \DIFadd{can, by virtue of energy-momentum }\DIFaddend conservation). The
\DIFdelbegin \DIFdel{latter takes into account that for $t\tbar$ production at hadron
colliders the }\DIFdelend \DIFaddbegin \DIFadd{decision to focus on TPs that contain both $n_t$ and $n_{\tbar}$
is rooted in the fact that the spins of pair-produced }\DIFaddend top and antitop \DIFdelbegin \DIFdel{are not polarized but their spins are
strongly correlated and observables combining the }\DIFdelend \DIFaddbegin \DIFadd{quarks
are highly correlated at hadron colliders 
(even though the quarks themselves are unpolarized). }{\bf \DIFadd{(KK: did I reword
  this correctly?)}}
\DIFadd{Observables that combine the }\DIFaddend decay products of \DIFaddbegin \DIFadd{the
}\DIFaddend $t$ and $\tbar$ will be sensitive to this spin
correlation\DIFaddbegin \DIFadd{~}\DIFaddend \cite{Bernreuther}.  A similar behaviour is expected in $t\tbar H$
production\DIFaddbegin \DIFadd{, }\DIFaddend where it can be shown that \DIFdelbegin \DIFdel{the }\DIFdelend single-spin asymmetries
vanish\DIFaddbegin \DIFadd{~}\DIFaddend \cite{Ellis,Biswas}. Hence, in order to construct observables
sensitive to the structure of the $tH$ coupling, we will \DIFdelbegin \DIFdel{prefer }\DIFdelend \DIFaddbegin \DIFadd{restrict our attention
to }\DIFaddend those
TPs that include information on the decay products of both \DIFaddbegin \DIFadd{the }\DIFaddend top and
anti-top quarks. Only five of the fifteen TPs in Eq.~(\ref{eq20}) do
not involve the \DIFaddbegin \DIFadd{four }\DIFaddend vector $q$ and, among these, \DIFdelbegin \DIFdel{we will focus on the
three
that }\DIFdelend \DIFaddbegin \DIFadd{only three
}\DIFaddend include both $n_t$ and $n_{\tbar}\,$\DIFdelbegin \DIFdel{:
}\DIFdelend \DIFaddbegin \DIFadd{.  Thus, we will restrict our attention
...
\DIFdelbegin \DIFdel{Finally}\DIFdelend \DIFaddbegin \DIFadd{Before turning to a consideration of various CP-odd observables}\DIFaddend ,
we remark that even though all \DIFaddbegin \DIFadd{of }\DIFaddend the above discussion \DIFdelbegin \DIFdel{was given
}\DIFdelend \DIFaddbegin \DIFadd{took place
}\DIFaddend within the context of $gg$-initiated production, similar conclusions
...
\label{sec3}
\DIFdelbegin \DIFdel{We present below }\DIFdelend \DIFaddbegin \DIFadd{In this section we present }\DIFaddend three types of observables based on the TPs discussed
in Sec.~\ref{sec2}, \DIFdelbegin \DIFdel{mean values, asymmetriesand angular
distributions }\DIFdelend \DIFaddbegin \DIFadd{namely, asymmetries, angular
distributions and mean values}\DIFaddend . These observables are sensitive not only to the
magnitude of the pseudoscalar coupling $\kpt$\DIFaddbegin \DIFadd{, }\DIFaddend but also to its
sign.  \DIFdelbegin \DIFdel{With the aim of testing them}\DIFdelend \DIFaddbegin \DIFadd{In order to test the various observables}\DIFaddend , we have
used \DIFdelbegin \DIFdel{in all the cases a
set of $10^5$ events }\DIFdelend \DIFaddbegin \DIFadd{$\mathtt{MadGraph5\_aMC@NLO}$ \mbox{%DIFAUXCMD
\cite{Madgraph} }%DIFAUXCMD
to simulate the process
}\DIFaddend $\ppprocess$ \DIFdelbegin \DIFdel{simulated }\DIFdelend at parton level \DIFdelbegin \DIFdel{with
$\mathtt{MadGraph5\_aMC@NLO}$ \mbox{%DIFAUXCMD
\cite{Madgraph} }%DIFAUXCMD
at a center-of-mass
energy of $14\,\mathrm{TeV}$ }\DIFdelend for different values of the couplings
$\kp$ and $\kpt$\DIFdelbegin \footnote{\DIFdel{Although we use the same number of events in
  all the cases, the required luminosities are different since the
  cross section depends on the value of $\kpt$.}}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{.  We have used this
somewhat large number of events
with the aim of
determining clearly
...
viability in Sec.~\ref{sec6}. Finally, we }\DIFdelend \DIFaddbegin \DIFadd{.  In all cases we have generated $10^5$ events
and have assumed a center-of-mass energy of
$14\,\mathrm{TeV}$.}\footnote{\DIFadd{Note that, since we generate
...
\DIFadd{We }\DIFaddend have also imposed the
following set of cuts: $p_T$ of leptons $> 10\,\mathrm{GeV}$, $|\eta|$
of leptons $< 2.5$, $|\eta|$ of b jets $< 2.5$ and $\Delta
R_{\ell\ell}>0.4$.  \DIFdelbegin %DIFDELCMD < \par %%%
\DIFdel{Under the assumption of no other new physics
sources but the inclusion of the }\DIFdelend \DIFaddbegin \DIFadd{Note that we have used this
...
}{\bf \DIFadd{(KK: I may not have worded this quite correctly.)}}

\DIFadd{Before continuing on to our analysis, let us make a few comments
regarding the values that we choose for $\kp$ and $\kpt$.
First of all, we note that if the }\DIFaddend pseudoscalar
coupling $\kpt$ \DIFdelbegin \DIFdel{,
indirect contraints based on the Higgs production from gluon fusionfollowed by its }\DIFdelend \DIFaddbegin \DIFadd{is the only source of physics beyond the SM,
then indirect contraints (based on Higgs production via gluon fusion,
followed by Higgs }\DIFaddend decay to two photons \DIFaddbegin {\bf \DIFadd{(KK: can we just say
  something like $gg\to H \to \gamma\gamma$?  Or is that not
  correct?)}}\DIFadd{) }\DIFaddend disfavour $\kp < 0$ \DIFdelbegin \DIFdel{without
resolving the degeneracy }\DIFdelend \DIFaddbegin \DIFadd{but do not
resolve the degeneracy }{\bf \DIFadd{(KK: did I reword this correctly?)}}
\DIFaddend in the sign of $\kpt$ \cite{Guadagnoli}. On
the other hand, \DIFdelbegin \DIFdel{by assuming }\DIFdelend \DIFaddbegin \DIFadd{if one assumes that }\DIFaddend the tensor structure of the
\DIFdelbegin \DIFdel{Lagrangian to
be }\DIFdelend \DIFaddbegin \DIFadd{Higgs interactions are }\DIFaddend the
same as \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{those of }\DIFaddend the SM and \DIFdelbegin \DIFdel{considering a parameterization with one }\DIFdelend \DIFaddbegin \DIFadd{if one parameterizes these interactions via
one }\DIFaddend universal Higgs coupling to vector bosons, $\kappa_V$, and one
universal Higgs coupling to fermions, $\kappa_f$, \DIFaddbegin \DIFadd{then }\DIFaddend the measured signal
strengths provided by \DIFaddbegin \DIFadd{the }\DIFaddend ATLAS and CMS collaborations are compatible with
the values predicted by the SM, (\DIFdelbegin \DIFdel{$\kappa_f=1,\kappa_V=1$).
Based on
these facts }\DIFdelend \DIFaddbegin \DIFadd{namely, $\kappa_f=1$ and $\kappa_V=1$).
With these facts in mind}\DIFaddend , we will\DIFaddbegin \DIFadd{, for the most part,
}\DIFaddend set the value of the scalar coupling to its SM
value \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend $\kp=1$\DIFdelbegin \DIFdel{, and concentrate in modifications of the value as well
as the sign of the pseudoscalar coupling }\DIFdelend \DIFaddbegin \DIFadd{) and will allow the pseudoscalar coupling to take on
various values (including both possible signs)}\DIFaddend . In particular, we
\DIFdelbegin \DIFdel{have
analized }\DIFdelend \DIFaddbegin \DIFadd{analyze }\DIFaddend the cases $\kpt=0,\pm 0.25, \pm 0.5, \pm 0.75,\pm 1$.
\DIFdelbegin \DIFdel{However, we will give some results and comments }\DIFdelend \DIFaddbegin \DIFadd{In addition, we also provide some analysis }\DIFaddend regarding the pure
$\mathrm{CP}$-odd \DIFdelbegin \DIFdel{hypothesis }\DIFdelend \DIFaddbegin \DIFadd{case }\DIFaddend ($\kp=0,\kpt=1$).
%\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
