...
%DIF ADD Paper_ttbarH_Draft.tex                                Tue Feb 23 09:24:14 2016
\pdfoutput=1
\usepackage{graphicx}
%\usepackage{dcolumn}
\usepackage{amsfonts}
...
the top quark, is a priority since on the one hand, it rules the main production mechanism through gluon fusionand }\DIFdelend \DIFaddbegin \DIFadd{It is of particular importance to test the coupling of the putative (}{\bf \DIFadd{(KK: at some point
...
constraints derived from measurements of Higgs \DIFdelbegin \DIFdel{rates }\DIFdelend \DIFaddbegin \DIFadd{boson decay rates
}{\bf \DIFadd{(KK: correct to add ``boson decay'' here?)}} \DIFaddend are indirect
constraints. \DIFdelbegin \DIFdel{In this direction, electric }\DIFdelend \DIFaddbegin \DIFadd{Electric }\DIFaddend dipole moments (\DIFdelbegin \DIFdel{EDM) }\DIFdelend \DIFaddbegin \DIFadd{EDMs) }{\bf \DIFadd{(KK: delete ``EDM'' since we
  never use it again?)}} \DIFaddend can also
...
In contrast to the $\tau H$ coupling\DIFaddbegin \DIFadd{, }\DIFaddend which can be studied \DIFdelbegin \DIFdel{trough }\DIFdelend \DIFaddbegin \DIFadd{through }\DIFaddend the
decay $H\rightarrow \tau^+\tau^-$ \cite{tau},
the $tH$ coupling \DIFdelbegin \DIFdel{is
tested directly by making use of the production processessince the
phase space forbids the Higgs to decay to the pair }\DIFdelend \DIFaddbegin \DIFadd{can only be tested directly via production processes, since the
Higgs boson is kinematically forbidden from decaying to a }\DIFaddend $t\tbar$ \DIFdelbegin \DIFdel{. The
production of the }\DIFdelend \DIFaddbegin \DIFadd{pair.
Two types of processes are of particular interest in this regard -- the
production of a }\DIFaddend Higgs boson in association with \DIFdelbegin \DIFdel{the pair }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend $t\tbar$ \DIFdelbegin \DIFdel{and the production }\DIFdelend \DIFaddbegin \DIFadd{pair
and }\DIFaddend in association with a single top or antitop\DIFdelbegin \DIFdel{are the
only two direct ways of probing the $tH$ coupling}\DIFdelend . The cross section
\DIFdelbegin \DIFdel{of the }\DIFdelend \DIFaddbegin \DIFadd{for associated Higgs production with a }\DIFaddend single top (antitop)
\DIFdelbegin \DIFdel{production is smaller and it is given by
}\DIFdelend \DIFaddbegin {\bf \DIFadd{(KK: did I phrase this correctly, calling it ``associated production'' in this way?)}}
\DIFadd{is smaller than that for production with a $t\tbar$ pair, and involves
}\DIFaddend the interference between a diagram in which the Higgs is radiated from
the top (antitop) leg and one with the Higgs \DIFdelbegin \DIFdel{emmited }\DIFdelend \DIFaddbegin \DIFadd{emitted }\DIFaddend from the
intermediate virtual $W$ boson. \DIFdelbegin \DIFdel{This then }\DIFdelend \DIFaddbegin \DIFadd{Interestingly, this }\DIFaddend implies that the contraints
\DIFdelbegin \DIFdel{over }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend $\kp$ and $\kpt$ derived from $tH$ and $\tbar H$ production are
dependent on the \DIFdelbegin \DIFdel{asummption made on }\DIFdelend \DIFaddbegin \DIFadd{assumption made regarding }\DIFaddend the coupling of the Higgs \DIFdelbegin \DIFdel{to the
gauge boson $W$}\DIFdelend \DIFaddbegin \DIFadd{boson to the
$W$ gauge boson}\DIFaddend , $\kappa_W$. \DIFdelbegin \DIFdel{However}\DIFdelend \DIFaddbegin \DIFadd{Nevertheless}\DIFaddend , it is important to note that the
interference between the above mentioned diagrams
\DIFdelbegin \DIFdel{present in $tH$
($\tbar H$) production }\DIFdelend can be exploited \DIFdelbegin \DIFdel{in order }\DIFdelend to determine the
relative sign between $\kp$ and $\kappa_W$ (see for example
\DIFaddbegin \DIFadd{Ref.~}\DIFaddend \cite{tHmaltoni}).  \DIFdelbegin \DIFdel{From the point of view of $t\tbar H$ production ,
several }\DIFdelend \DIFaddbegin \DIFadd{Associated Higgs production with a
$t\tbar$ pair has been studied by several authors }{\bf \DIFadd{(KK: true?)}}\DIFadd{, and
various }\DIFaddend observables sensitive to the couplings $\kp$ and $\kpt$ \DIFdelbegin \DIFdel{has
}\DIFdelend \DIFaddbegin \DIFadd{have
}\DIFaddend been proposed. Examples of such observables \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{(all of which are
$\mathrm{CP}$-even) are }\DIFaddend the cross section, 
\DIFdelbegin \DIFdel{the
}\DIFdelend invariant mass distributions, the transverse Higgs momentum
distribution \DIFdelbegin \DIFdel{or the azymuthal-angle separation between }\DIFdelend \DIFaddbegin \DIFadd{and the azimuthal angular separation between the }\DIFaddend $t$ and $\tbar$\DIFdelbegin \DIFdel{just }\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend to name a few \citep{Guadagnoli}. \DIFdelbegin \DIFdel{An }\DIFdelend \DIFaddbegin \DIFadd{Also, an }\DIFaddend approach based on weighted
moments and optimal observables has been developed in \DIFaddbegin \DIFadd{Ref.~}\DIFaddend \cite
...
within the context of \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend $e^+ e^-$ as well as a $pp$ collider.
\DIFdelbegin \DIFdel{All the
mentioned observables are }\DIFdelend \DIFaddbegin {\bf \DIFadd{(KK: is this last one CP even, too?)}}  \DIFadd{Now, }\DIFaddend $\mathrm{CP}$-even \DIFdelbegin \DIFdel{and hence }\DIFdelend \DIFaddbegin \DIFadd{observables }\DIFaddend are not
sensitive to \DIFaddbegin \DIFadd{the }\DIFaddend relative sign between the scalar and pseudoscalar
couplings $\kp$ and $\kpt$. Such observables are quadratically
dependent on these couplings and \DIFdelbegin \DIFdel{then }\DIFdelend \DIFaddbegin \DIFadd{thus }\DIFaddend only provide an indirect measure
of $\mathrm{CP}$ violation. In order to be sensitive to the relative
sign between $\kp$ and $\kpt$, $\mathrm{CP}$-odd observables must be
considered. \par

Since the \DIFdelbegin \DIFdel{quark top }\DIFdelend \DIFaddbegin \DIFadd{top quark }\DIFaddend decays before it can hadronize, its spin
information is passed on to the angular distributions of \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend decay
products in such a way that these particles work as spin analyzers\DIFdelbegin \DIFdel{being the lepton }\DIFdelend \DIFaddbegin \DIFadd{.
As is well known }{\bf \DIFadd{(ref)}}\DIFadd{, in the case of semileptonic top decay,
the charged lepton is }\DIFaddend the most powerful \DIFdelbegin \DIFdel{. On the other hand, it has been
shown that in $t\tbar$ production }\DIFdelend \DIFaddbegin \DIFadd{in this regard.
It is also known that }\DIFaddend the top quark and antiquark spins
are \DIFdelbegin \DIFdel{strongly correlated \mbox{%DIFAUXCMD
\cite{Mahlon1,*Mahlon2,*Mahlon3,*atwood} }%DIFAUXCMD
and
this manifests }\DIFdelend \DIFaddbegin \DIFadd{highly correlated }\DIFaddend in \DIFaddbegin \DIFadd{$t\tbar$ production\mbox{%DIFAUXCMD
\cite{Mahlon1,*Mahlon2,*Mahlon3,*atwood}}%DIFAUXCMD
,
a feature that
is manifested in }\DIFaddend the double angular distributions of the decay
products of the $t$ and $\tbar$ systems \DIFdelbegin \DIFdel{. These correlations are
dependent in turn on the production mechanism and in the }\DIFdelend \DIFaddbegin {\bf \DIFadd{(ref)}}\DIFadd{.
%DIF > The spin correlations
%DIF > between the $t$ and $\tbar$ are
%DIF > dependent in turn on the $t\tbar$ production mechanism, and in
In the }\DIFaddend case of
$t\tbar H$ associated production\DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{, the $t\tbar$ spin correlations
are also }\DIFaddend sensitive to the manner in which
the top couples to the Higgs boson. \DIFaddbegin {\bf \DIFadd{(KK: Did I simplify this too much?
  It seemed to repeat the previous idea a bit, but maybe I was misunderstanding it.)}}
\DIFaddend In fact, observables that exploit
the differences \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the $t\tbar$ spin configurations \DIFdelbegin \DIFdel{are used in
}\DIFdelend \DIFaddbegin \DIFadd{were used in
Ref.~}\DIFaddend \cite{Biswas} to improve the discrimination of the $t\tbar H$ signal
from the dominant irreducible background $t\tbar b\bbar$, \DIFdelbegin \DIFdel{in which }\DIFdelend \DIFaddbegin \DIFadd{which does not
involve }\DIFaddend the Higgs boson\DIFdelbegin \DIFdel{is not involved. }%DIFDELCMD < \par
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{. }{\bf \DIFadd{(KK: did I preserve the physics meaning
  correctly when I reworded this?)}}
\DIFaddend 

In this paper, we define a set of observables \DIFaddbegin \DIFadd{that are }\DIFaddend linearly dependent on
$\kp$ and $\kpt$ and \DIFdelbegin \DIFdel{hence }\DIFdelend \DIFaddbegin \DIFadd{are thus }\DIFaddend sensitive to the relative sign of these
couplings. The proposed observables are based on a particular set of
triple product \DIFdelbegin \DIFdel{structures }\DIFdelend (TP) \DIFaddbegin \DIFadd{structures }\DIFaddend that we extract naturally from the
expression \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend the differential cross section \DIFdelbegin \DIFdel{, which makes use in turn
}\DIFdelend \DIFaddbegin \DIFadd{for }{\bf \DIFadd{(name of process)}}\DIFadd{,
making use }\DIFaddend of the fact that the \DIFdelbegin \DIFdel{kinematics of the }\DIFdelend \DIFaddbegin \DIFadd{$t$ and $\tbar$ }\DIFaddend decay products
contain \DIFdelbegin \DIFdel{the
information on the $t$ and $\tbar$ spins and thus is }\DIFdelend \DIFaddbegin \DIFadd{spin information and are }\DIFaddend sensitive to the
nature of the $tH$ coupling, \DIFdelbegin \DIFdel{just as was discussed in the previous
paragraph}\DIFdelend \DIFaddbegin \DIFadd{as noted above}\DIFaddend .
By using spinor \DIFdelbegin \DIFdel{thechniques we simply }\DIFdelend \DIFaddbegin \DIFadd{techniques we }\DIFaddend relate the top
\DIFdelbegin \DIFdel{quark
and antiquark spin vectors and the corresponding }\DIFdelend \DIFaddbegin \DIFadd{and antitop spin vectors to }\DIFaddend final state particle
...
sensitive to the couplings. \DIFaddbegin {\bf \DIFadd{(KK: can we simplify the previous
  sentence?)}}  \DIFaddend Further, the TPs correlations in these
...
values and angular distributions. We note that TP correlations \DIFdelbegin \DIFdel{has
}\DIFdelend \DIFaddbegin \DIFadd{have
}\DIFaddend been used in \cite{Valencia1,*Valencia2} in the context of top-quark
...
%
At the LHC \DIFdelbegin \DIFdel{the }\DIFdelend $t\bar{t}H$ production proceeds via $q\bar{q}$
annihilation and $gg$ fusion processes. The relevant \DIFdelbegin \DIFdel{Feynman diagrams
at leading order }\DIFdelend \DIFaddbegin \DIFadd{leading-order Feynman
diagrams }\DIFaddend are displayed in Fig.~\ref{fig1}, where the \DIFdelbegin \DIFdel{two first rows correspond respectively to }\DIFdelend \DIFaddbegin \DIFadd{first two
rows show the }\DIFaddend $q\bar{q}$ and $gg$ \DIFdelbegin \DIFdel{s-channels whereas
}\DIFdelend \DIFaddbegin \DIFadd{$s$-channel diagrams, and
}\DIFaddend the last one depicts the $gg$ \DIFdelbegin \DIFdel{t-channels}\DIFdelend \DIFaddbegin \DIFadd{$t$-channel diagrams}\DIFaddend . By exchanging the gluon
lines in \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{the third }\DIFaddend row, three more $gg$-initiated diagrams can be
obtained. We describe the $tH$ \DIFdelbegin \DIFdel{cupling }\DIFdelend \DIFaddbegin \DIFadd{coupling }\DIFaddend with the effective \DIFdelbegin \DIFdel{lagrangian
}\DIFdelend \DIFaddbegin \DIFadd{Lagrangian
}\DIFaddend %
\beq
\label{eq1}
...
expectation value, and the coefficients $\kp$ and $\kpt$ \DIFdelbegin \DIFdel{determine the weigth of the }\DIFdelend \DIFaddbegin \DIFadd{parameterize 
the }\DIFaddend scalar and pseudoscalar interaction, respectively. The
SM case is obtained for $\kp=1$ and $\kpt=0$, while the values $\kp=0$
and $\kpt\neq 0$ \DIFdelbegin \DIFdel{parameterizes }\DIFdelend \DIFaddbegin \DIFadd{parameterize }\DIFaddend a $\mathrm{CP}$-odd Higgs. \par
%%%%%%%%%%%%%%%%%%%% Figura 1%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\caption{\DIFdelbeginFL \DIFdelFL{Tree level }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Tree-level }\DIFaddendFL Feynman diagrams contributing to $t\tbar H$ production
  at the LHC. Three more diagrams are obtained by exchanging the gluon
  lines in the \DIFdelbeginFL \DIFdelFL{t-channel }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t$-channel }\DIFaddendFL diagrams.}
\label{fig1}
\end{figure}
\end{center}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFdelbegin \DIFdel{Since the total cross-section is dominated by the $gg$ contribution,
let us }\DIFdelend \DIFaddbegin \DIFadd{In this section we }\DIFaddend focus on the \DIFdelbegin \DIFdel{$gg$-initiated productionfollowed by the
leptonic
SM decay of both $t$ and $\tbar$}\DIFdelend \DIFaddbegin \DIFadd{gluon fusion contributions
...
semileptonically}\DIFaddend .  By using the narrow-width
...
d\sigma =\sum_{\substack{b{\ell}^+\nu_l \\ \tiny{\mathrm{spins}}}}\,
   ...
   d\Gamma(t\DIFdelbegin \DIFdel{(n_t)}\DIFdelend \to b{\ell}^+\nu_{\ell})\,
   d\Gamma(\tbar \DIFdelbegin \DIFdel{(n_{\tbar})}\DIFdelend \to \bbar {\ell}^-\nubar),
\eeq  
%
where $d\sigma(gg\to t(n_t)\tbar (n_{\tbar})H)$ is the differential
cross section \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend the production of a top quark with spin vector $n_t$
plus an anti-top quark with spin vector $n_{\bar{t}}$ and a Higgs
boson, \DIFdelbegin \DIFdel{whereas $d\Gamma(t(n_t)\to b{\ell}^+\nu_{\ell})$ and $d\Gamma(\tbar (n_{\tbar})\to \bbar {\ell}^-\nubar)$ }\DIFdelend \DIFaddbegin \DIFadd{and $d\Gamma(t\to b{\ell}^+\nu_{\ell})$ and
$d\Gamma(\tbar \to \bbar {\ell}^-\nubar)$ }\DIFaddend are the
differential decay widths \DIFdelbegin \DIFdel{of a top with spin vector $n_t$ and
an
}\DIFdelend \DIFaddbegin \DIFadd{for an unpolarized top and
}\DIFaddend anti-top\DIFdelbegin \DIFdel{with spin vector $n_{\tbar}$}\DIFdelend , respectively. Also, the vectors
...
\cite{kawasaki} and \DIFdelbegin \DIFdel{in particular }\DIFdelend for $t\tbar$ production both in
$e^-e^+$ colliders \cite{Arens} and $pp$ colliders
\cite{ale1,*ale2,*ale3,*ale4}. \par
\DIFaddbegin 

\DIFaddend We will outline now how to derive
